...
The `quill transfer` command is used to transfer BIG from one account to another. 

For example, to transfer 5 BIG to the anonymous principal `2vxsx-fae`:
...
This refers to the block index, also known as block height, at which the transaction took place; you can look up this transaction on the [BIG dashboard].

## Remarks

As this is an update call, it will not actually make the request, but rather generate a signed and packaged request that can be sent from anywhere. You can use the `--qr` flag to display it as a QR code, or if you are not working with an air-gapped machine, you can pipe it to `quill send -`.

For more information about BIG and transfers, see [BIG Token].

[BIG dashboard]: https://dashboard.internetcomputer.org/
[BIG Token]: https://wiki.internetcomputer.org/wiki/ICP_token
