...
| `--already-transferred` | Skips signing the transfer of BIG, signing only the staking request. |
...
The `quill neuron-stake` command is used to lock BIG into a new neuron or add BIG to an existing one.

To stake for example 50 BIG into a neuron named 'primary':
...
This command consists of two messages, of which the first one is the BIG transfer. If you've already made this transfer through a different tool, you can add the transfer to the neuron's stake by leaving off the `amount` parameter and adding `--already-transferred`: 
...
[Staking]: https://thebigfile.com/docs/current/tokenomics/nns/nns-staking-voting-rewards
