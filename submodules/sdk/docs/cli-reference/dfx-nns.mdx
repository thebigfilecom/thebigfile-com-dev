...
#### Example: Accessing BIG on the command line
Two accounts in the local ledger is initialized with BIG that can be used for testing.  One uses a secp256k1 key, which is convenient for command line usage, another uses an ed25519 key, which is more convenient in web applications.



To use BIG on the command line:
...
To use BIG in an existing web application:
...
```
* That identity can now make API calls, including sending BIG.
