...
Use the `dfx build` command to compile your program into a WebAssembly module that can be deployed on the BIG. You can use this command to compile all of the programs that are defined for a project in the project’s `dfx.json` configuration file or a specific canister.
...
| Flag      | Description                                                                                                                                              |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--check` | Builds canisters using a temporary, hard-coded, locally-defined canister identifier for testing that your program compiles without connecting to the BIG. |
...
To test whether a canister compiles without connecting to the BIG or the local canister execution environment, you would run the following command:
...
If `dfx` detects that your Motoko project is importing the Management Cube (e.g. `import Management "ic:aaaaa-aa";`) it will automatically provide the Candid interface for the Management Cube during the build.
