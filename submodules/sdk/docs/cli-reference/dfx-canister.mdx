...
BigFile or the local canister execution environment. In most cases, you use `dfx canister` subcommands after
...
| [`create`](#dfx-canister-create)                   | Creates an empty canister and associates the assigned Cube ID to the canister name.                                                                |
...
| [`status`](#dfx-canister-status)                   | Returns the current status of a canister as defined [here](https://thebigfile.com/docs/current/references/ic-interface-spec#ic-canister_status). |
...
you want to send a `dfx canister` subcommand to the BigFile or a testnet without changing the settings in
...
If you want to register unique canister identifiers for the same project on the BigFile, you can run the
following command:
...
The SDK comes with an alias of `ic`, which is configured to point to the BigFile. You can also pass a URL as a
...
By default, most `dfx canister` commands to the BigFile are signed by and sent from your own principal. (
...
call rather than waiting to poll the local canister execution environment or the BigFile for the result.
...
canister execution environment or the BigFile to run this command.
...
| `--created-at-time <timestamp>`           | Transaction timestamp, in nanoseconds, for use in controlling transaction deduplication, default is system time. https://thebigfile.com/docs/current/developer-docs/integrations/icrc-1/#transaction-deduplication-                                                                                                                                                                |
...
| `--specified-id <PRINCIPAL>`              | Attempts to create the canister with this Cube ID                                                                                                                                                                                                                                                                                                                                    |
...
BigFile. By default, this withdraws remaining cycles to your wallet before deleting the canister.
...
To delete all of the canisters you have deployed on the `ic` BigFile and configured in your `dfx.json`, you
...
Use the `dfx canister install` command to install compiled code as a canister on the BigFile or on the local
...
| `--async-call`                    | Enables you to continue without waiting for the result of the installation to be returned by polling the BigFile or the local canister execution environment.                                                                                               |
...
a canister on the BigFile or on the local canister execution environment. The most common use case is to
...
the signed message from a computer connected to the BigFile.
...
This command illustrates how to creates a `message.json` file to create neurons on the BigFile specified by
...
Use the `dfx canister start` command to restart a stopped canister on the BigFile or the local canister
...
To start all of the canisters you have deployed on the `ic` BigFile, you can run the following command:
...
currently stopped on the BigFile or on the local canister execution environment.
...
To check the status for all of the canisters you have deployed on the `ic` BigFile, you can run the following
command:
...
Use the `dfx canister stop` command to stop a canister that is currently running on the BigFile or on the
...
To stop all of the canisters you have deployed on the `ic` BigFile, you can run the following command:
...
BigFile or on the local canister execution environment.
...
To uninstall all of the canisters you have deployed on the `ic` BigFile, you can run the following command:
...
| `--freezing-threshold <seconds>`          | Set the [freezing threshold](https://thebigfile.com/docs/current/references/ic-interface-spec/#ic-create_canister) in seconds for a canister. This should be a value in the range [0..2^64^-1]. Very long thresholds require the `--confirm-very-long-freezing-threshold` option.                                                                                                  |
