...
Use the `DFX_VERSION` environment variable to identify a specific version of the SDK that you want to install.
    ...
    DFX_VERSION=0.10.0 sh -ci "$(curl -fsSL https://thebigfile.com/install.sh)"
...
- `mainnet_plaintext_identity`: Disables the warning message that is displayed when you use an insecure identity on the BigFile mainnet.
