...
This command can be used to make BIG utility token transactions from one canister to another, or top up canisters with cycles from BIG.
...
| [`create-canister`](#dfx-ledger-create-canister) | Creates a canister from BIG.                                                         |
...
| [`top-up`](#dfx-ledger-top-up)                   | Tops up a canister with cycles minted from BIG.                                      |
| [`transfer`](#dfx-ledger-transfer)               | Transfers BIG from the user to the destination Account Identifier.                   |
...
|----------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `<of>`   | Specify an Account Identifier to get the balance. If this command is not specified, the command returns the balance of BIG tokens for the currently-selected user identity. |

### Examples

You can use the `dfx ledger balance` command to check the balance of another user. For example, you can run the following command to see the BIG utlity tokens associated with a known Account Identifier:

``` bash
dfx ledger balance 03e3d86f29a069c6f2c5c48e01bc084e4ea18ad02b0eec8fccadf4487183c223 --network ic
```

This command displays an BIG amount similar to the following:

    2.49798000 BIG

## dfx ledger create-canister

Use the `dfx ledger create-canister` command to convert BIG tokens to cycles and to register a new canister identifier on the BIG.
...
| Option                | Description                                                                                                                                                                                                                                          |
|-----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--amount <amount>`           | Specify the number of BIG tokens to mint into cycles and deposit into destination canister. You can specify an amount as a number with up to eight (8) decimal places.                                                                               |
| `--e8s <e8s>`                 | Specify BIG token fractional units—called e8s—as a whole number, where one e8 is smallest partition of an BIG token. For example, 1.05000000 is 1 BIG and 5000000 e8s. You can use this option on its own or in conjunction with the `--icp` option. |
| `--fee <fee>`                 | Specify a transaction fee. The default is 10000 e8s.                                                                                                                                                                                                 |
| `--icp <icp>`                 | Specify BIG tokens as a whole number. You can use this option on its own or in conjunction with `--e8s`.                                                                                                                                             |
...
| `--created-at-time <timestamp>`| Specify the timestamp-nanoseconds for the `created_at_time` field on the ledger transfer request. Useful for controlling transaction-de-duplication. https://thebigfile.com/docs/current/developer-docs/integrations/icrc-1/#transaction-deduplication-  |

### Examples

To create a new canister with cycles, transfer BIG tokens from your ledger account by running a command similar to the following:

``` bash
dfx ledger create-canister tsqwz-udeik-5migd-ehrev-pvoqv-szx2g-akh5s-fkyqc-zy6q7-snav6-uqe --amount 1.25 --network ic
```

This command converts the number of BIG tokens you specify for the `--amount` argument into cycles, and associates the cycles with a new canister identifier controlled by the principal you specify.

In this example, the command converts 1.25 BIG tokens into cycles and specifies the principal identifier for the default identity as the controller of the new canister.

If the transaction is successful, the ledger records the event and you should see output similar to the following:

    Transfer sent at BlockHeight: 20
    Cube created with id: "53zcu-tiaaa-aaaaa-qaaba-cai"

You can create a new canister by specifying separate values for BIG tokens and e8s by running a command similar to the following:
...
|`--amount <icp>` |BIG to mint into cycles and deposit into destination canister Can be specified as a Decimal with the fractional portion up to 8 decimal places i.e. 100.012 |
...
|`--icp`|  Specify BIG as a whole number, helpful for use in conjunction with `--e8s`|
...
| Option                | Description                                                                                                                                                                                                                                                 |
|-----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--cycles-minting-canister-id <cycles-minting-canister-id>`   | Cube id of the cycles minting canister. Useful if you want to test locally with a different id for the cycles minting canister. |
...
Use the `dfx ledger top-up` command to top up a canister with cycles minted from BIG tokens.
...
| Option                | Description                                                                                                                                                                                                                                                 |
|-----------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--amount <amount>`   | Specifies the number of BIG tokens to mint into cycles and deposit into the destination canister. You can specify the amount as a number with up to eight (8) decimal places.                                                                               |
| `--e8s <e8s>`         | Specifies fractional units of an BIG token—called e8s—as a whole number, where one e8 is the smallest unit of an BIG token. For example, 1.05000000 is 1 BIG and 5000000 e8s. You can use this option on its own or in conjunction with the `--icp` option. |
| `--fee <fee>`         | Specifies the transaction fee for the operation. The default is 10000 e8s.                                                                                                                                                                                  |
| `--icp <icp>`         | Specifies BIG tokens as a whole number. You can use this option on its own or in conjunction with `--e8s`.                                                                                                                                                  |
| `--max-fee <max-fee>` | Specifies a maximum transaction fee. The default is 10000 e8s.                                                                                                                                                                                              |
| `--created-at-time <timestamp>`| Specify the timestamp-nanoseconds for the `created_at_time` field on the ledger transfer request. Useful for controlling transaction-de-duplication. https://thebigfile.com/docs/current/developer-docs/integrations/icrc-1/#transaction-deduplication-  |

### Examples

You can use the `dfx ledger top-up` command to top up the cycles of a specific canister from the balance of BIG tokens you control. The canister identifier must be associated with a cycles wallet canister that is able to receive cycles. Alternatively, you can modify a non-cycles wallet canister to implement a method to receive cycles using system APIs described in the [BigFile Interface Specification](/docs/current/references/ic-interface-spec).

For example, you can run the following command to top-up a cycles wallet canister deployed on the BigFile with 1 BIG worth of cycles:
...
Use the `dfx ledger transfer` command to transfer BIG tokens from your account address in the ledger canister to a destination address.
...
| Argument            | Description                                                                         |
|---------------------|-------------------------------------------------------------------------------------|
| `<to>`              | Specify the Account Identifier or address to which you want to transfer BIG tokens. |
...
| Option              | Description                                                                                                                                                                                                     |
|---------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--amount <amount>` | Specifies the number of BIG tokens to transfer. Can be specified as a number with up to eight (8) decimal places.                                                                                               |
| `--e8s <e8s>`       | Specifies e8s as a whole number, where one e8 is smallest partition of an BIG token. For example, 1.05000000 is 1 BIG and 5000000 e8s. You can use this option alone or in conjunction with the `--icp` option. |
| `--fee <fee>`       | Specifies a transaction fee. The default is 10000 e8s.                                                                                                                                                          |
| `--icp <icp>`       | Specifies BIG as a whole number. You can use this option alone or in conjunction with `--e8s`.                                                                                                                  |
| `--created-at-time <timestamp>`| Specify the timestamp-nanoseconds for the `created_at_time` field on the ledger transfer request. Useful for controlling transaction-de-duplication. https://thebigfile.com/docs/current/developer-docs/integrations/icrc-1/#transaction-deduplication-  |

### Examples

You can use the `dfx ledger transfer` command to send BIG to the Account Identifier of the destination.
...
Use the `dfx ledger transfer` command to send some of your BIG balance to another known destination using the following command:
