...
Use the `dfx cache delete` command to delete a specified version of the BIG SDK from the version cache on the local computer.
...
You can use the `dfx cache delete` command to permanently delete versions of the BIG SDK that you no longer want to use. For example, you can run the following command to delete the BIG SDK version `0.6.2`:
...
Use the `dfx cache install` command to install the BIG SDK using the version currently found in the `dfx` cache.
...
Use the `dfx cache list` command to list the BIG SDK versions you have currently installed and used in projects.

If you have multiple versions of the BIG SDK installed, the cache list displays an asterisk (\*) to indicate the currently active version.
...
You can use the `dfx cache list` command to list the BIG SDK versions you have currently installed and used in projects. For example, you can run the following command to list versions of the BIG SDK found in the cache:

``` bash
dfx cache list
```

This command displays the list of the BIG SDK versions found similar to the following:
...
Use the `dfx cache show` command to display the full path to the cache used by the BIG SDK version you are currently using.
...
You can use the `dfx cache show` command to display the path to the cache used by the BIG SDK version you are currently using:

``` bash
dfx cache show
```

This command displays the path to the cache used by the BIG SDK version you are currently using:
