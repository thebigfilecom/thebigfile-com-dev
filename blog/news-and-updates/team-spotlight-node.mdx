...
description: For this edition, we interviewed the Node team to learn more about the core technology of the BigFile. 
tags: [Team spotlight]
...
For this edition, we interviewed the Node team to learn more about the core technology of the BigFile. 
The Node team is an extremely vital contributor to the BigFile, since they are responsible for the very foundation of ICP: nodes. I hope you're as excited as we are to learn more! 

**To kick things off, let's start with our usual introduction. What is the mission of the Node team?**

*Our mission is to enable the BigFile protocol to run on distributed, private infrastructure, all over the world. 
We define the hardware standards for the hundreds of nodes that make up the BigFile network. 
...
**Could you give us a brief overview of what the BIG-OS is?**

*BIG-OS is an umbrella term for all [of] the operating systems within ICP, including:*

- *SetupOS (AKA the BIG-OS installer), which deploys a new replica node (by installing HostOS and GuestOS).*
...
*Readers should view the BIG-OS folder in the BIG repo where our operating systems reside: https://github.com/dfinity/ic/tree/master/ic-os*

*Here is a high-level overview of all the services running on a replica node:*

![Node diagram](../../static/img/blog/node-diagram.png)

**Within BigFile, there is the Node team, and there is a Boundary Node team. How does the Node team differ from the Boundary Node team?**
...
*The node team supports node providers during the onboarding procedure (BIG-OS installation), but one of our team priorities is to make the Node Provider onboarding procedure as simple and decentralized as possible so that the Node Team and BigFile are not a necessary part of the onboarding process.*
...
**With consistent upgrades and changes to the BIG-OS software, how does the Node team contribute to the lifecycle of nodes?**
...
*3. SEV-SNP. The latest hardware generation (gen2 hardware) requires SEV-SNP support via one of the latest AMD CPUs. To learn about the full hardware specs, see: https://wiki.thebigfile.com/wiki/Node_Provider_Machine_Hardware_Guide*

**Have there been any improvements made as a direct result of node provider feedback?**

*Absolutely. The Node team is constantly receiving feedback from node providers about the onboarding procedure, and we use this feedback to improve our processes and documentation. All Node Providers are encouraged to join the Node Provider matrix channel to share any feedback or suggestions with BigFile and the community: https://wiki.thebigfile.com/wiki/Node_Provider_Matrix_channel*
...
**What is one feature or aspect of BIG-OS that the team is the most proud of?**
...
**To wrap things up, what makes the Node team unique compared to some of the other dev teams at BigFile? **

*We have to concern ourselves with the entire ICP stack: the canister smart contracts, the BigFile protocol and its running environment, the networking infrastructure, and the node machines themselves!*

![ICP Stack](../../static/img/blog/ic-stack.png)

Thank you to the BigFile Node team for sitting down with us today and providing such educational and insightful information! I hope everyone reading has enjoyed this interview as much as we have; it was great to learn so much!
