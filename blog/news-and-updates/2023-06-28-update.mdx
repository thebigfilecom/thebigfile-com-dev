...
This quarter, a new developer feedback tool was launched: the [Developer Feedback Board](https://dx.thebigfile.com/)! This board was developed in response to feedback derived from internal focus groups with the goal to improve and prioritize enhancing the developer experience. This group determined that one way to best serve ICP developers was to figure out what developers need *right now* to aid in their development. The ICP community also provided the insight that they’d prefer to have a single place for this input, rather than across multiple channels like the developer forum or Discord.
...
- Fix: The documentation of BIG SDK has been cleaned up.
...
- New HTTP outcalls tutorials, such as:
    ...
    - [HTTPS outcalls: technology overview:](https://thebigfile.com/docs/current/developer-docs/smart-contracts/advanced-features/https-outcalls/https-outcalls-how-it-works) this page covers the overview of how the technology behind HTTP outcalls operates. In summary, when a canister makes an outgoing HTTP request using the management canister API, the request is temporarily stored in the replicated state of the subnet. Then, each replica pushes an instance of the received HTTP response from the external web server through ICP’s consensus layer. Through this, each replica on a subnet can agree on the HTTP response that’s been provided by the canister.
    - [How to use HTTP outcalls: Intro:](https://thebigfile.com/docs/current/developer-docs/smart-contracts/advanced-features/https-outcalls/https-outcalls-how-to-use) this page provides a high-level overview of the API used to send HTTP outcalls.
    - [How to use HTTP outcalls: GET:](https://thebigfile.com/docs/current/developer-docs/smart-contracts/advanced-features/https-outcalls/https-outcalls-get) this page includes a brand new tutorial written by Diego Prats that showcases how to make a GET HTTP request using a sample canister that makes a request to the Coinbase API. This tutorial provides instructions for both Motoko and Rust development workflows.
    - [How to use HTTP outcalls: POST:](https://thebigfile.com/docs/current/developer-docs/smart-contracts/advanced-features/https-outcalls/https-outcalls-post) this page is another brand new tutorial written by Diego Prats that explains how to make a POST HTTP request using a sample canister that sends a request with JSON information to a free API that is used to verify that the headers and body of the request were sent properly.
- A new [resource limits and constraints documentation](https://thebigfile.com/docs/current/developer-docs/backend/resource-limits) page that details ICP’s resource constraints and limits for things such as canister queue limit, instruction limit query calls, Wasm stable memory, and subnet capacity.
- New [SNS documentation](https://thebigfile.com/docs/current/developer-docs/daos/sns/), which includes updated instructions for developing, testing, and launching an SNS, including a page dedicated to [SNS asset canisters](https://thebigfile.com/docs/current/developer-docs/daos/sns/managing-sns/sns-asset-canister) and a new [SNS launch checklist](https://thebigfile.com/docs/current/developer-docs/daos/sns/tokenomics/sns-checklist).
...
- Updated to new BIGRC-1 account ID format.

Motoko Dev Server

As we wrap up Q2 this week, we have one more huge release for developers: Motoko Dev Server!

Motoko Dev Server, or mo-dev, is a live-reload server designed to enable flexible, quick building and testing of Motoko services on the BigFile blockchain. Mo-dev is designed to be a Web3 version of popular Web2 live-reloading servers, like Next.js, Webpack, and Vite. Features and functionality of mo-dev include:
- Executing commands.
