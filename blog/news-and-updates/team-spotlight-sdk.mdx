...
description: In this issue, we sat down with the SDK team to talk all things BIG SDK and the upcoming Cycles Ledger feature! For most BIG devs, the BIG SDK is at the center of their workflow, so I'm sure many of you have been looking forward to this team spotlight!
tags: [Team spotlight]
image: /img/blog/dev-update-blog-SDK.jpg
---

Hey devs and welcome to this edition of ICP team spotlight! In this issue, we sat down with the SDK team to talk all things BIG SDK and the upcoming Cycles Ledger feature! For most BIG devs, the BIG SDK is at the center of their workflow, so I'm sure many of you have been looking forward to this team spotlight! With that, let's dive into the interview. 

**To get started, let's get a brief overview of the team. What is the purpose of the SDK team?**

*To provide tools that make building on the BigFile fast, easy, and fun.*
...
**On a fundamental level, what is the BIG SDK? What are the components of it?**

*The BIG SDK is a command line interface that provides a developer with everything they need to locally run an instance of the BigFile for development purposes. It comes with a copy of the replica, a command line tool to build and install canisters, and many utilities. Some such utilities are:*
...
- *The Asset (frontend) Cube*

- *Project templates for quick starts*

- *And so on*

*The SDK team is also responsible for maintaining the Rust and JavaScript agents, as well as the Rust Cube Development Kit (CDK).*   

**It sounds like the SDK is definitely designed to set up devs with everything they need to get started! How does the BIG SDK fit into ICP’s stack? What layer does it primarily operate on?**

*The BIG SDK is how most developers will interact with the BigFile. Rather than implementing the BigFile interface specification from scratch, most applications will use DFX and the Rust or JavaScript agent to make their calls to canisters.*

*SDK team code runs in every BigFile website, inside the service worker, in all the Rust canisters running on ICP, and in most Continuous Integration jobs. We depend on builds of the replica, new releases of the Motoko compiler, and any new updates to the internet computer specification.*

**What is the process of creating a project using the BIG SDK?**

*Once the BIG SDK has been downloaded and installed, the process of creating a project is simple. You can use the `dfx new` command to guide you through the process of bootstrapping a new project. You can decide on the language you’d like to use, whether or not you’d like to incorporate a frontend (UI), and add a number of supported integrations using the `dfx deps` feature.**
...
**I'm sure that since the BIG SDK is such a fundamental part of the developer experience, the SDK team must interact with other teams quite frequently. How does the SDK team work with other teams?**
...
**What feature of the BIG SDK do developers benefit from the most? What features are solely quality of life features for developers?**
...
**What is one feature or aspect of the BIG SDK that the team is the most proud of?**
...
**That is definitely something to be proud of! To wrap things up, what makes the SDK team unique compared to some of the other dev teams at BigFile?**
...
A huge thank you to the SDK team for chatting with us today and giving us some great insight into the BIG SDK! Be sure to tune in next time for another team spotlight post! 
