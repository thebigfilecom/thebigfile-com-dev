...
The BIGRC-4 token standard is nearing finalization! BIGRC-4 will specify a standard for processing  batch transfers for tokens compliant with BIGRC-1. The motivation behind this standard is to facilitate optimized batch transfers within DeFi applications that commonly use multi-party settlements or token distributions.

BIGRC-4 will extend the BIGRC-1 standard to enable multiple subaccounts owned by a principal to transfer tokens to multiple recipient accounts within a single ledger call. This method will provide reduced latency and cost when compared to traditional multi-account token transfers.

Check out the [BIGRC-4 standard](https://github.com/skilesare/BIGRC/blob/main/ICRCs/BIGRC-4/BIGRC-4.md) and participate in the discussion about BIGRC-4 on the [developer forum](https://forum.dfinity.org/t/icrc4-batch-transfers-nearing-finalization-please-review/27395/2)
...
- Cube logs can now be called using a query call to the method `fetch_canister_logs`.
