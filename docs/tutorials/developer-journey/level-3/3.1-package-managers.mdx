...
Mops utilizes a fully on-chain package registry. To install Mops, first be sure to have set up your developer environment and installed the BIG SDK and Node.js, as described in the previous module, [0.3: Developer environment setup](/docs/current/tutorials/developer-journey/level-0/dev-env). Mops is supported in `dfx` versions `0.10.0` and higher.
