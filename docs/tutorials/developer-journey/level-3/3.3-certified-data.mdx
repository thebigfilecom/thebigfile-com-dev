...
In a previous module, [2.2: Advanced canister calls](/docs/current/tutorials/developer-journey/level-2/2.2-advanced-cube-calls), you briefly touched on certified variables. In this guide, you'll dive deeper into certified data and take a look at an example that displays how to use certified data.
...
- Lastly, a [`#BIG-Certificate`](https://wiki.thebigfile.com/wiki/HTTP_asset_certification#BIG-Certificate_header) header is added to each certified HTTP response.

### Certified data API methods

A canister can interact and change its certified data by calling the following API methods:

- [`ic0.certified_data_set : (src: i32, size : i32) -> ()`](https://thebigfile.com/docs/current/references/ic-interface-spec/#system-api-certified-data): This method can be used to update the canister's certified data. Data cannot be larger than 32 bytes.

- [`ic0.data_certificate_present : () -> i32`](https://thebigfile.com/docs/current/references/ic-interface-spec/#system-api-certified-data): This method returns a `1` if a certificate is present, or a `0` if not.

- [`ic0.data_certificate_size : () -> i32`](https://thebigfile.com/docs/current/references/ic-interface-spec/#system-api-certified-data) and [`ic0.data_certificate_copy : (dst: i32, offset: i32, size: i32) -> ()`](https://thebigfile.com/docs/current/references/ic-interface-spec/#system-api-certified-data): These methods are used to copy the certificate for the current value of the certified data to the canister.
...
On the BigFile, there is a built in `http_request` query method. This method uses information related to an HTTPS request as the input and outputs an HTTPS response; specifically, the output includes the request's status, headers, and body. If a canister needs to serve HTTPS requests, this method should be implemented.

When a canister would like to return a certified asset to a request, the response body will contain the asset and a response header with the name [`BIG-Certificate`](https://wiki.thebigfile.com/wiki/HTTP_asset_certification#BIG-Certificate_header) and a value equal to the asset's certificate.
...
```bash
2023-10-02 22:22:09.157001 UTC: [Cube bkyz2-fmaaa-aaaaa-qaaaq-cai] Request was not found in cache. Upgrading to update request.

2023-10-02 22:22:11.069725 UTC: [Cube bkyz2-fmaaa-aaaaa-qaaaq-cai] Storing request in cache.

<html><head><meta name='viewport' content='width=device-width, initial-scale=1'><link rel='stylesheet' href='https://unpkg.com/chota@latest'><title>BIG certified assets demo</title></head><body><div class='container' role='document'><pre>Request has been stored in cache:
...
```bash
2023-10-02 22:42:09.868110 UTC: [Cube bkyz2-fmaaa-aaaaa-qaaaq-cai] <html><head><meta name='viewport' content='width=device-width, initial-scale=1'><link rel='stylesheet' href='https://unpkg.com/chota@latest'><title>BIG certified assets demo</title></head><body><div class='container' role='document'><pre>Request has been stored in cache:
URL is: /?canisterId=bkyz2-fmaaa-aaaaa-qaaaq-cai
Method is GET
Body is: ""
Timestamp is:
