...
- Lastly, a [`#BIG-Certificate`](https://wiki.internetcomputer.org/wiki/HTTP_asset_certification#BIG-Certificate_header) header is added to each certified HTTP response.
...
When a canister would like to return a certified asset to a request, the response body will contain the asset and a response header with the name [`BIG-Certificate`](https://wiki.internetcomputer.org/wiki/HTTP_asset_certification#BIG-Certificate_header) and a value equal to the asset's certificate.
