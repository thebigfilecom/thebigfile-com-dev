...
```bash
dfx --identity ic_admin canister call access_hello_backend greet "BigFile Admin"
```
...
```bash
(
    "Hello, BigFile Admin. You have a role with administrative privileges.",
)
```
...
Cube smart contracts can be configured to execute automatically on a periodic schedule or after a specified delay. To configure this, there are two ways to schedule canister execution:
...
That'll wrap up level 3 of our developer journey! In the next level, you'll start by taking a look at using the BIG ledger.
