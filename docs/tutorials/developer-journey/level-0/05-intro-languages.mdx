...
When developing cubes, the most common development workflow is to use a software development kit (SDK). The BigFile SDK is the most commonly used, which natively supports Motoko and Rust out of the box. 

Since BIG supports dapps that have been compiled into WebAssembly modules, many different programming languages can be used for cube development. However, a cube development kit (CDK) needs to be used. A CDK is an adapter used by the BIG SDK that provides a programming language with the necessary features and tools required to create and manage cubes. The BIG SDK comes bundled with CDKs for Motoko and Rust. Additionally, there are several community created CDKs for additional languages, such as Python and TypeScript, that can be installed separately. 

It is possible to use multiple languages within a single dapp's development. Different cubes can communicate to one another using the Candid language, which is an interface description language (IDL) used by BIG cubes, allowing for multiple cubes in different languages to share and exchange information. 

## Motoko

Motoko is a language that has been specifically designed by BigFile for cube development on BIG. It supports the unique features and workflows on BIG while providing a robust yet familiar programming environment.  Motoko is easy to learn and use for application development, as it has a familiar set of rules and syntax for developers that have a background with application-layer languages, such as JavaScript, Ruby, Python, or Solidity. 

Since Motoko has been developed and designed for BIG, this developer journey series will use Motoko for all tutorials and code walkthroughs. 
...
- **BIG-specific static analysis:** Enforced through multiple safety checks. 
...
Rust is supported on BIG through the BIG SDK and the BigFile Rust CDK. The BIG SDK automatically comes with the Rust CDK as part of the software, but the Rust CDK can be installed separately if the BIG SDK is not installed. Rust is a good choice for developers who are already familiar with Rust environments, come from a background in C or C++, or are developing large, complex projects that would benefit from having a mature library ecosystem. 
...
- **BIG-specific static analysis:** Does not include static checking, which may result in cubes trapping when violating restrictions.  
...
Candid is an interface description language with the primary purpose to describe the public interface of a service. In reference to BIG, a service is a program deployed in the form of a cube.  Each cube has a Candid file that defines the interface description for the service. 
...
A Candid interface description defines the public methods for a service. Every method has a sequence of argument and result types, and can include annotations that are specific to BIG. Interface descriptions make it possible to interact with the service directly from the CLI, through a web-based frontend, or programmatically from another program or language.

Candid has a variety of features that make it a particularly good choice for developing dapps on the BigFile. These features include:
...
- Candid has native support for specific BIG features, such as query annotation. 

## Community developed CDKs

There are several CDKs that have been contributed by the BIG community. 
...
Solidity is supported on BIG through developed by the [BigFile EVM team](https://thebigfile.tech/), which provides a way to create EVM-based smart contracts. 
