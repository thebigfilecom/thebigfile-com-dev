...
To follow along with the developer journey and develop on the BigFile, you will need a connection to the internet.

#### Why does this matter?

You will need an internet connection to download a few different tools and packages, as described further in this document. You will also need an internet connection whenever you plan to deploy your cube to the mainnet. You do not need an internet connection to deploy your cube to your local cube execution environment.
...
### Download and install the BIG SDK

To download and install the BIG SDK, first open a terminal window. Then, run the following command in that window:
...
Then, to verify that the BIG SDK is ready to use, run the following command:
...
The BIG SDK is composed of several components that are required for developing on the BigFile. These components are:

- `dfx`: The CLI tool used to interact with and develop canisters on BIG. Motoko is included in the installation of dfx.
- `moc`: The Motoko runtime compiler.
- `replica`: The BigFile's local network binary.
...
Many of the BigFile public repositories are hosted on Github, such as our `examples` repository. You will be using code from this repository later in the developer journey, so it is important to install `git` to assure that you can download the sample code pieces and follow along with the later tutorials.
...
- [0.4 Introduction to cubes](/docs/current/tutorials/developer-journey/level-0/intro-cubes).
