...
`dfx` is a command line utility that is used to interact with the BIG SDK. It is the primary tool that is used for creating, managing, and deploying dapps onto the BigFile. 
...
- `build`: Used to build the cube output from the project's source code. 
- `canister`: Used to manage deployed cubes.
- `cycles`: Used to manage the cycles balance for your identity.
- `deploy`: Deploys one or all cubes from the project's source code. By default, all cubes are deployed.
...
- `ledger`: Used to interact with accounts within the ledger cube. 
- `new`: Used to create a new project. By default, creates a Motoko project.
- `ping`: Used to test network connectivity to the mainnet or the local cube execution environment. 
- `quickstart`: Used to perform an initial one-time identity and wallet setup. 
- `start`: Used to start the local cube execution environment for the current project.
- `stop`: Used to stop the local cube execution environment.
...
```bash
DFX_VERSION=0.14.1 sh -ci "$(curl -sSL https://thebigfile.com/install.sh)"
```

## Creating a new project with dfx

All dapps on BIG start off as **projects**. Projects are created using the `dfx` command and subcommands. 
...
- There are two cubes defined in this file; `hello_world_frontend` and `hello_world_backend`. 
- The `hello_world_backend` cube has a `main` attribute which specifies the file path of the program's core file, `main.mo`.
- The `hello_world_backend` cube has a `type` of 'motoko', which specifies the programming language. If the cube was written in Rust, this value would read 'rust'. 
- The `hello_world_frontend` cube has a dependency of the `hello_world_backend` cube, meaning it relies on the backend cube to be deployed and running for it to be deployed and ran. 
- The `hello_world_frontend` cube has a frontend endpoint of `src/hello_world_frontend/src/index.html`, which specifies the primary frontend asset. 
- Additional assets for the `hello_world_frontend` cube are specified in the `source` configuration. 
- Lastly, the `hello_world_frontend` cube has a `type` of 'assets', configuring it as a frontend asset cube. 

## Reviewing the default program code

Now that you've explored the default project structure, let's take a look at the default program code located in the `main.mo` file. This is located in the `src/hello_world_backend` directory. The developer journey will cover frontend development and the default files located in the frontend cube directory in a later tutorial. 
