...
Candid is an interface description language that is used to describe the public interface of a service. A **service** is usually in the form of an application deployed as a cube on BIG. Public interfaces of cubes are used to interact with the cube. Candid supports interactions through the BIG SDK via the terminal, through a web browser, or through the use of agents. It also provides a way to specify input argument values and display return values from different cube **methods**, regardless of the manner used to interact with the cube. Recall that a method is a piece of code specifying a task which declares a sequence of arguments and their associated result types.

Candid is language-agnostic, which is a key benefit since it allows for the inter-operation between cubes written in different languages, such as Motoko, Rust, and JavasScript, and client applications.

Candid is unique since it provides functionality and features not available in other technologies, such as JSON or XML. Some of these features include the evolution of service interfaces, direct value mapping for Candid values to the types and values of the cube's language, and the ability to pass complex data such as references to services and methods. Candid also has built-in support for BIG-specific features like query annotation.

Up until now in this developer journey, you've utilized the Candid UI a few times through the web browser. If you recall, in several tutorials you've interacted with the backend cubes in the web browser with the URL from an output such as:
...
Candid supports the following set of types in order to provide a natural mapping of data types based on reasonable choices suited for each cube language:
...
Typically, as a developer you may not work with your program data as Candid values; instead, you may work with a host language such as Javascript and utilize Candid to transparently transport your dapp's data to your cube written in Motoko or Rust. The receiving cube treats this data as native Motoko or Rust values.
...
The Candid files you have used thus far in the developer journey have been generated automatically. This is because a Motoko cube's Candid files are automatically generated by the Motoko compiler when the cube is compiled, then stored in the project's `/declarations` directory. When Candid files are auto-generated, it is not recommended to edit them manually, and any changes made to the file will be overwritten by `dfx` the next time the project is built and deployed. For cubes written in other languages, the Candid file must be written manually or generated using an external tool.
...
:::info
Generating Candid files is supported for cubes written in Motoko. Cubes written in other languages will need to have their Candid files written manually. Check out the [Candid interface description](/docs/current/references/candid-ref) for more information on manually writing the service description.
:::

The following is an example Motoko cube that defines simple `add`, `subtract`, and `get` functions:
...
This example Candid interface description helps to illustrate that every method has a sequence of argument and result types. Methods can also include annotations, like the query notation shown in this example (`() -> (int) query;`), which is a feature specific to BIG.

For cubes written in other languages such as Rust, you can develop your service using the language's native types. [View Rust examples of how to write Candid service descriptions](https://thebigfile.com/docs/current/developer-docs/backend/rust/candid/).

To generate Candid files from Rust code, the Rust CDK `v0.11.0` and higher support auto-generation using the [`candid-extractor` tool](https://thebigfile.com/docs/current/backend/candid/generating-candid).
...
You'll be using the sample Motoko cube that you looked at in the [generating service descriptions](#generating-service-descriptions) section earlier. Open the `src/candid_example_backend/main.mo` file in your code editor and replace the existing code with the following:
...
Now that your cube is deployed, let's use `dfx` to interact with the methods defined by your Candid file. This section of tutorial will look familiar, since in previous tutorials you've interacted with cubes in the same manner - you just hadn't known you were directly interacting with the Candid defined services since the Candid files had been automatically generated for you in the background.

For example, let's call the `add` method to add a number to our counter with the command:

```bash
dfx cube call candid_example_backend add 12
```

Then, you can call the `get` method to return the value you just added:

```bash
dfx cube call candid_example_backend get
(12 : int)
```
...
```bash
dfx cube call candid_example_backend subtract 4
```
...
```bash
dfx cube call candid_example_backend subtract 8
```


### Interacting with a service using a web browser

Based on the types of services offered by a cube, Candid can provide a web interface, known as the Candid UI, that allows the developer to call cube functions for debugging and testing from within a web browser. This functionality does not require the development of any frontend code and does not require a frontend cube be created or deployed.

To interact with the Candid UI from a web browser, recall that when a cube is deployed, the following output is returned:
...
In this output, you can see the URL for the Candid interface under 'Backend cube via Candid interface'. Opening this URL in a web browser will display the following UI:
...
Next, let's explore how to test and debug projects and cubes.
