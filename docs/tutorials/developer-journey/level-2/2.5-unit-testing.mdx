...
### Cube unit testing
In addition to the `Suite`, `Testable` and `Matchers` packages, Motoko Matchers also contains a `Cube` package. The `Cube` package can be used to execute unit tests for cubes. Below is an example that uses this `Cube` package:

```motoko
import Cube "cube:CanisterName";
import C "mo:matchers/Cube";
import M "mo:matchers/Matchers";
import T "mo:matchers/Testable";
...
- First, it imports a cube called `CanisterName`. This allows the result of functions within the cube to be tested.

- Then, it imports the `Cube`, `Testable` and `Matchers` packages from the Motoko Matchers project.

- A `batchSize` is defined; this determines how many times the test will be run.

- Two tests are defined. Both test the result of the cube's `greet` function:

    - The first tests if the cube's greeting function is passed the text 'Bob', does the result text equal 'Hello, Bob!'?
...
Want to look at a more complex, running example? You can take a look at the unit tests that are used in the [invoice cube example](https://github.com/dfinity/invoice-cube/tree/main/test).
...
[Learn more about Rust unit testing](https://mmapped.blog/posts/01-effective-rust-cubes.html).

You may also consider the [BigFile Rust guidelines on testing](https://docs.dfinity.systems/dfinity/spec/meta/rust.html#_tests).

## Tests using PocketIC

PocketIC is a community-developed project that provides local cube testing using a Python library. Using PocketIC, developers can write tests for their cube in Python, then deploy the scripts to interact with their local BIG replica. An example test with PocketIC might look like:
...
# use PocketIC to interact with your cubes
...
:::info
Need the full `package.json` file? [Check out the repo containing the full project for this tutorial](https://github.com/krpeacock/sample-cube-e2e).
:::
...
:::caution
You'll be diving a bit into using an **agent** in this portion of the tutorial. Note that the developer journey has briefly mentioned agents thus far, but hasn't fully explored agents yet. The developer journey will cover agents in a future tutorial, but for this tutorial it is important to know that an agent a library used to make calls to the BigFile public interface. You'll be using it to communicate the cube's public methods, defined in the `declarations/e2e_tests_backend/e2e_tests_backend.did.js`, to the file containing our the code for our tests.
:::
...
- Sets up file handlers by reading the cube IDs from their associated JSON file.
...
Next let's deploy the cube and run our test with the commands:
...
Now let's create a more complex test that checks the cube's metadata using the `CanisterStatus` API. Insert the following code at the end of the `src/tests/e2e_tests_backend.test.ts` file:
...
:::info
Need the full `src/tests/e2e_tests_backend.test.ts` file? [Check out the repo containing the full project for this tutorial](https://github.com/krpeacock/sample-cube-e2e).
:::

This test simply checks that our cube's metadata contains a Candid interface.
...
This tells the code to automatically start dfx and deploy the project's cube.
