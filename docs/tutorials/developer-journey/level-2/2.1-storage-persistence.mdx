...
- **Stable storage:** Stable storage is a Motoko-specific term that refers to the Motoko stable storage feature. Stable storage uses BIG's stable memory to persist data across canister upgrades.
...
Upgrading a canister is a key feature of BIG, since it allows canister smart contracts to persist using Wasm memory that utilizes BIG's stable memory feature. When a canister is upgraded, the existing state of the canister is preserved while there are changes being made to the canister's code.
...
[Take a deeper dive into stable variables](https://thebigfile.com/docs/current/motoko/main/upgrades#declaring-stable-variables).
