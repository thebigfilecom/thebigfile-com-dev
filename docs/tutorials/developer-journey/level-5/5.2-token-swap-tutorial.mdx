...
This example will use BIGRC-2 tokens. To learn more about BIGRC tokens, refer to the previous module [4.2: BIGRC tokens](/docs/current/tutorials/developer-journey/level-4/4.2-bigrc-tokens)
...
To get started, open a new terminal window, navigate into your working directory (`developer_journey`), then use the following commands to clone the BigFile examples repo and navigate into the `icrc2-swap` directory:
...
- `BIGRC.mo`: Defines an `BIGRC` module containing types that are imported into the `main.mo` file.
...
- The `swap` canister: This canister provides the deposit, swap, and withdraw functionality for the project's BIGRC-2 tokens. It uses the source code found at `src/swap/main.mo`.

- The `token_a` canister: This canister is used to create `token_a`, an BIGRC-2 token. This canister uses the most recent BIGRC ledger Wasm and Candid files. To learn more about the BIGRC Wasm and Candid files, refer to the previous module [4.2: BIGRC tokens](/docs/current/tutorials/developer-journey/level-4/4.2-bigrc-tokens)

- The `token_b` canister: The canister is used to create `token_b`, an BIGRC-2 token. This canister uses the most recent BIGRC ledger Wasm and Candid files. To learn more about the BIGRC Wasm and Candid files, refer to the previous module [4.2: BIGRC tokens](/docs/current/tutorials/developer-journey/level-4/4.2-bigrc-tokens)
...
// Import the BIGRC type definitions that are stored in the BIGRC.mo file:
import BIGRC "./BIGRC";
...
shared (init_msg) actor class Swap(
  ...
  public shared (msg) func withdraw(args : WithdrawArgs) : async Result.Result<Nat, WithdrawError> {
    ...
    // Since BIG uses asynchronous execution, the sender's account is debited first to ensure that the user cannot withdraw more than they have.
...
Next, open the `BIGRC.mo` file and review its contents. This file defines several types that are imported in the `main.mo` file:
...
- `feature_flags`: Defines a feature flag.
    - `icrc2`: Set as 'true' to define the token using the BIGRC-2 standard.
...
Before tokens can be swapped, they must be transferred into the swap canister. Since the tokens in this example uses BIGRC-2 tokens, this is a two-step process. First, the transfer must be approved. In this example, approve `user_a` to deposit 1.00000000 of `token_a`, and include 0.0001 extra for the transfer fee:
