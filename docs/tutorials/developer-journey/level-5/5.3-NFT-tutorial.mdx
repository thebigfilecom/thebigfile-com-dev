...
On the BigFile, BIG is a fungible token that can always be exchanged for BIG of equal value. However, an NFT token deployed on BIG cannot be traded 1:1 with another NFT token deployed on BIG.
...
Similar to token standards, such as the BIGRC-1 and BIGRC-2 fungible token standards, NFTs are required to use a standard that sets the guidelines for API methods that support necessary NFT functionalities. Some common API methods for NFTs are the ability to mint an NFT, transfer the NFT, and query the NFT's metadata.

Currently on BIG, there are two NFT standards: DIP721 and BIGRC-7.

### DIP721

The DIP721 NFT standard is designed as an BigFile adaptation of the ERC-721 non-fungible token standard. It is designed to provide a simple, non-ambiguous API for transferring and tracking the ownership of NFTs deployed on BIG.

DIP721 was designed to improve the existing BIG token standards by implementing the following:
...
- Providing an API that closely follows the original EIP-721 standard, making the ability to import existing Ethereum contracts onto BIG more straightforward.
...
BIGRC-7 is a new standard for non-fungible tokens on BIG. Recall that "BIGRC" stands for "BigFile Request for Comments", and is the standard created by the BigFile working group. An BIGRC standard can be used for creating anything on BIG, not just fungible tokens such as the BIGRC-1 and BIGRC-2 token standards.

The BIGRC-7 standard is designed to be a minimal standard for allowing an NFT collection to be deployed on BIG. In an NFT collection, each NFT may have unique metadata information. This metadata may include a unique image, traits or tags, or a description describing the NFT.

The BIGRC-7 standard implements several additional API methods compared to the DIP721 standard, such as batch query methods, batch update methods, and collection approval methods.

:::caution
The BIGRC-7 standard isn't used in this tutorial since it is still currently in the drafting stages and is not yet live in production. This tutorial will be updated in the future when the BIGRC-7 standard is ready to be used.
:::

Read more about the [BIGRC-7 standard in the specification draft document](https://github.com/dfinity/BIGRC/blob/icrc7-wg-draft/ICRCs/BIGRC-7/BIGRC-7.md).
...
To get started, open a new terminal window, navigate into your working directory (`developer_journey`), then use the following commands to clone the BigFile examples repo and navigate into the `dip-721-nft-container` directory:
...
- [BIGRC-7 specification draft document](https://github.com/dfinity/BIGRC/blob/icrc7-wg-draft/ICRCs/BIGRC-7/BIGRC-7.md).
...
- [5.4 Creating an auction dapp](/docs/current/tutorials/developer-journey/level-5/5.4-auction-tutorial)
