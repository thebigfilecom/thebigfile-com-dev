...
By creating an auction on a decentralized platform such as BIG, there is an immutable record of the users who bid on the item and who purchased the item. This allows for verifiable records for sales of high-value items or digital assets such as NFTs.
...
actor {
  ...
  type Bid = {
    /// Define the price for the bid using BIG as the currency:
    price : Nat;
...
From the auction details screen, you can place a bid on the auction under 'New bid'. Enter the amount you'd like to bid, then select 'Bid BIG'.
...
- [5.5 Next steps](/docs/current/tutorials/developer-journey/level-5/5.5-next-steps)