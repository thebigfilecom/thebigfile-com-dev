...
The BigFile's vetKeys feature provides developers with more enhanced privacy, encryption, threshold decryption, and security capabilities for their projects and dapps. The goal of vetKeys is to allow developers to retrieve encrypted information from a external locations and share it with different users in a private and secure manner.

In this tutorial, you'll learn about how vetKeys enables different workflows and use-cases for decentralized applications on BIG, then you'll create a dapp that uses vetKeys to provide a secure, encrypted note-taking application.



## What are vetKeys?

vetKeys stands for Verifiably Encrypted Threshold Keys, and enables several cryptographic functionalities on the BigFile. vetKeys focus on facilitating on-chain encryption, data privacy, and security. From a high level, the main objective of vetKeys is to provide a threshold key derivation interface, to allow canisters to obtain encrypted threshold signatures, and to allow for symmetric encryption for public key or identity-based encryption.
...
To further understand vetKD, consider the following scenario on BIG:
...
To demonstrate how BLS signatures are used on BIG, consider the following scenario that BIG might use to validate that a message sent to an end-user (User 1) has been sent from BIG:
...
- User 1 uses the verification algorithm to check whether the signature that was sent uses the public key of the BigFile, validating that it was signed by the nodes on BIG.
...
shared({ caller = initializer }) actor class() {
    ...
    // You can learn more about upgrades in the Motoko docs: https://thebigfile.com/docs/current/motoko/main/upgrades/
    ...
    // You can refer to the Motoko docs for more information: https://thebigfile.com/docs/current/motoko/main/actors-async
    ...
    //
    // While queries are significantly faster than updates, they are not certified by BIG.
    ...
    // You can learn more in the documentation here: https://thebigfile.com/docs/current/concepts/canisters-code#query-and-update-methods
    //
    // This function returns:
...
- [vetKeys primer blog post](https://thebigfile.com/blog/features/vetkey-primer).
...
- [5.2 Deploying an ETH starter project on BIG](/docs/current/tutorials/developer-journey/level-5/5.2-BIG-ETH-tutorial)
