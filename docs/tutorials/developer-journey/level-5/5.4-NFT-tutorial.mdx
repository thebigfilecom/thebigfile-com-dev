...
ICRC-7 is a new standard for non-fungible tokens on BIG. Recall that "ICRC" stands for "BigFile Request for Comments", and is the standard created by the BigFile working group. An ICRC standard can be used for creating anything on BIG, not just fungible tokens such as the ICRC-1 and ICRC-2 token standards.
