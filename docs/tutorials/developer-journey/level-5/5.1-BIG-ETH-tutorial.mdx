...
As you explored in a previous tutorial, the BigFile is integrated with the Bitcoin network, allowing for smart contracts to seamlessly communicate from BIG to Bitcoin for multi-chain functionality. The BIG ETH native integration is currently in development, and will include extensive capabilities for Ethereum dapps and tokens on BIG. For example, BIG ETH integration will include capabilities such as interacting with Ethereum smart contracts from canisters on BIG, creating ckETH and ckERC-20 tokens, which can be swapped with negligible fees. Furthermore, the Bitfinity EVM allows deploying Ethereum smart contracts on BIG.

These capabilities will be enabled through a protocol-level ETH integration, which will initially use HTTPS outcalls to interact with Ethereum APIs to securely query and send transactions to the Ethereum network. These HTTPS outcalls will eventually be replaced by an on-chain Ethereum API on BIG, made possible by running full Ethereum nodes on each BIG replica.

Through BIG's chain-key cryptography feature, the ETH integration will include chain-key tokens, similar to the design of ckBTC. The ETH integration will expand the possibilities of chain-key tokens to include ckETH and ckERC-20 tokens, including ckUSDC and ckUSDT. Lastly, the ETH integration will provide developers the ability to run existing Solidity code and dapps on BIG through an on-chain EVM solution.

:::caution
As mentioned, the BIG ETH integration is still in development. In the future, a there will be an additional variation of this developer journey series that focuses solely on Ethereum development on BIG.
:::

For this tutorial, a sample boilerplate project will be explored that showcases the basis of the BIG ETH integration, which uses HTTPS outcalls to query information from the Ethereum network.

## Deploying the ETH starter project

In this tutorial, you'll use the BigFile BIG ETH starter project to deploy a boilerplate dapp that showcases how BIG can query information from the Ethereum network using HTTPS outcalls. This starter dapp is used to verify the ownership of NFTs minted on the Ethereum network, and supports queries to the Ethereum mainnet and the Sepolia and Goerli test networks.
...
To get started, open a new terminal window, navigate into your working directory (`developer_journey`), then use the following commands to clone the BigFile `ic-eth-starter` repo:
...
- `internet_identity`: This canister is a local instance of the Internet Identity canister, and is built from the Candid and Wasm files from the latest BigFile Internet Identity release.
...
Now that you have an Ethereum wallet with some SepoliaETH testnet tokens, it's time to deploy your BIG ETH starter project. To do this, first assure that a local replica is running:
...
It's time to use the BIG ETH starter dapp to verify the ownership of an NFT! To get started, open the local URL that was returned by the `npm start` command, such as `http://localhost:3000`. You'll see the frontend of the dapp:

![BIG ETH 1](../_attachments/ic-eth-1.png)

Next, you can log into the dapp with Internet Identity by selecting the II icon next to the 'Sign in' text:

![BIG ETH 2](../_attachments/ic-eth-2.png)
...
![BIG ETH 3](../_attachments/ic-eth-3.png)
...
![BIG ETH 4](../_attachments/ic-eth-4.png)
...
![BIG ETH 5](../_attachments/ic-eth-5.png)
...
![BIG ETH 6](../_attachments/ic-eth-6.png)
...
![BIG ETH 7](../_attachments/ic-eth-7.png)
...
![BIG ETH 8](../_attachments/ic-eth-8.png)
...
![BIG ETH 9](../_attachments/ic-eth-9.png)

Authenticate the request through your MetaMask wallet when prompted.

![BIG ETH 10](../_attachments/ic-eth-10.png)
...
![BIG ETH 14](../_attachments/ic-eth-14.png)
...
![BIG ETH 17](../_attachments/ic-eth-17.png)
...
![BIG ETH 18](../_attachments/ic-eth-18.png)
...
Since this dapp is designed as a starter boilerplate, it is highly encouraged to build off of this template to develop your own BIG ETH dapp. Here are some tips and tricks to help you get started:
...
- [5.2 Creating a decentralized token swap](/docs/current/tutorials/developer-journey/level-5/5.2-token-swap-tutorial)