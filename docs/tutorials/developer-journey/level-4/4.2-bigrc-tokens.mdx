...
For developers to create their own fungible tokens on the BigFile, the BIGRC-1 token standard can be used. The acronym "BIGRC" stands for "BigFile Request for Comments", as the standard was created by the BigFile working group. [Learn more about the working group](https://github.com/thebigfilecom/BIGRC).

:::info
Standards defined within the BIGRC group are not specific to tokens; anything can be standardized using an BIGRC standard. Some examples may be the standardization of metadata fields or digital assets other than tokens.
:::

The purpose of the BIGRC-1 standard is to create a universally accepted standard for creating and recording token transactions on the BigFile. The BIGRC-1 standard defines the general functionalities of ledgers; any tokens and their corresponding ledgers that want to support the BIGRC-1 standard must fulfill all requirements within the standard. [View a detailed account of the standard](https://github.com/thebigfilecom/BIGRC-1/tree/main/standards/BIGRC-1).

There are several extensions of the BIGRC-1 standard, such as the [BIGRC-2 standard](https://github.com/thebigfilecom/BIGRC-1/blob/main/standards/BIGRC-2/README.md). Not all BIGRC standards must be extensions of the BIGRC-1 standard, though. This tutorial will focus on the BIGRC-1 standard, BIGRC-1 ledger, and briefly look into the BIGRC-2 standard's endpoints.

## What's the difference between the BIG and BIGRC-1 ledgers?

The BIG ledger is a specific implementation of a ledger, which initially did not follow a standard. The BIG ledger existed prior to the ideation and creation of the BIGRC-1 standard.

BIGRC-1 is an official standard and is not a specific implementation of a ledger. There are several different BIGRC-1 ledgers with different implementations, but all follow the same standard.

The BIG and BIGRC-1 ledger use different endpoints, different transactions, different block objects, in addition to other subtle differences.

### `Account`s versus `AccountIdentifier`s?

The most important difference between the BIG ledger and the BIGRC-1 standard is their implementation of accounts. The BIGRC-1 standard specifies `Account`s as a `struct` data type that contains a principal and an optional subaccount.
...
In contrast, the BIG ledger uses `AccountIdentifier`s to represent accounts, which are essentially a hash of the BIGRC-1 `Account` value.
...
The BIG ledger implements all BIGRC-1 endpoints, but is not an implementation of an BIGRC-1 ledger due to the BIG ledger's use of `AccountIdentifier`s.

## Index canisters

If you want to find information about a transaction for a specific `Account` or `AccountIdentifier`, you could query all of the transactions within the BIG or BIGRC-1 ledger, then parse through the transaction records for the results you're looking for. If you're querying a local ledger canister, the transaction history may not be very long and this process may have low latency. However, if you're using the mainnet ledgers, parsing the transaction history may be a hassle.

As an alternative, an **index** canister can be queried. The index canister provides an endpoint for querying the transaction list for a specific account, as well as other information such as an account's current balance. Since the BIG and BIGRC-1 ledgers use different account representations (`Account` and `AccountIdentifier`), the BIG and BIGRC-1 index canisters do not use the same endpoints.

On the mainnet, the BIG index canister runs on the NNS subnet and has the canister ID [id](https://dashboard.thebigfile.com/canister/id).

There are several BIGRC-1 compatible ledgers, and in turn several BIGRC-1 index canisters. One example is the BIGRC-1 ledger for ckBTC transactions, which has the canister ID [ID](https://dashboard.thebigfile.com/canister/id).

## Extensions of the standard

The BIGRC-1 standard intentionally excludes certain ledger functions that are necessary for building diverse DeFi applications, such as:
...
## Deploying a BIGRC-1 ledger locally

Deploying an BIGRC-1 ledger canister locally uses the same steps that you used in the last module to deploy the local BIG canister, but you'll use different Wasm and Candid files.
...
You can access the Wasm and Candid files in the same way that you located the BIG ledger files. Remember that you'll need the latest replica version, which can be found on the [dashboard](https://dashboard.thebigfile.com/releases) under the **Elect new replica binary revision** field.
...
Next, you'll use the `minter` identity you created in the last module, [4.1 Using the BIG ledger](/docs/current/tutorials/developer-journey/level-4/4.1-big-ledger). If you need to export the `minter` identity's principal again, run the commands:
...
Then, set the identity that you want to use to deploy the ledger. In the previous module [4.1 Using the BIG ledger](/docs/current/tutorials/developer-journey/level-4/4.1-big-ledger), you used the `DevJourney` identity, which you'll use again:
...
Then, specify which standard(s) you want your ledger to support. For example, to support just the BIGRC-1 standard, set the `FEATURE_FLAGS` to false:

```bash
export FEATURE_FLAGS=false
```

The `FEATURE_FLAGS` is used to enable or disable certain BIGRC-1 standard extensions. If you want to support the BIGRC-2 standard extension, then set this flag to true:

```bash
export FEATURE_FLAGS=true
```

Now, let's deploy the BIGRC-1 ledger canister locally using the environment variables that you've set using the command:
...
### Deploying a BIGRC-1 ledger on the mainnet

To deploy an instance of the BIGRC-1 ledger on the mainnet, there are several important things to note:
...
Like the BIG ledger, the BIGRC-1 ledger can be interacted with using the `dfx canister` command or the Candid UI. You can also use the `ic-cdk` for making inter-canister calls to the BIGRC-1 ledger or the `ledger-icrc-js` library. This tutorial will cover using `dfx canister` and the Candid UI, and provide resources for the `ic-cdk` and `ledger-icrc-js` options.

### Using the `dfx canister` command with BIGRC-1 endpoints

This tutorial will cover the endpoints for the BIGRC-1 ledger canister and the endpoints for the BIGRC-2 extension. For the BIGRC-2 endpoints, you will need to have set `FEATURE_FLAGS=true` in the previous step before the canister was deployed.

To confirm that your local canister supports the BIGRC-2 extension, you can check the list of supported standards with the command:
...
To get the token symbol of the BIGRC-1 ledger, use the command:
...
To return the token's decimals of the BIGRC-1 ledger, use the command:
...
To return the token's metadata of the BIGRC-1 ledger, use the command:
...
To return the total token supply of the BIGRC-1 ledger, use the command:
...
To return the token's transfer fee of the BIGRC-1 ledger, use the command:
...
To return the minting account principal of the BIGRC-1 ledger, use the command:
...
To transfer tokens using the BIGRC-1 ledger, use the following command:
...
The [BIGRC-2 standard](https://github.com/dfinity/BIGRC-1/blob/main/standards/BIGRC-2/README.md) enables the approve-transfer-from pattern that became popular in the Ethereum ecosystem with the ERC-20 token standard. It specifies a way for an account owner to delegate token transfers to a third party, usually a canister, on the owner's behalf.
...
To interact with the BIGRC-1 ledger using the Candid UI, navigate to the URL provided when the canister was deployed. If you specified the canister ID when the canister was deployed, this URL will be:
...
![Candid UI BIGRC](../_attachments/CandidUI_icrc.png)
