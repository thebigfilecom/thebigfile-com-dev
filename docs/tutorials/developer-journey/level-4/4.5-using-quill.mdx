...
Now that you've covered both the BIG ledger and the NNS, it's time to take a look at a CLI tool that can be used to interact with both. Quill is a ledger and governance toolkit that provides support for self-custody of BIG or ckBTC tokens and functionality to interact with the NNS, SNSs, and BIG ledger from a **cold wallet**.
...
In this tutorial, you'll take a look at how to use quill for workflows like interacting with the BIG ledger, sending or receiving ckBTC, and interacting with the NNS and neurons.

## What is quill?

Quill is a CLI tool used for interacting with BIG's governance and ledger canisters. A unique feature of quill is the ability to create signed messages on an offline computer, or a 'cold wallet'. Once these messages are created, they must be transferred to an online computer, however, in order for them to take effect on the mainnet. Since the BigFile uses cryptographically verifiable messages, messages can be created on an offline computer then transmitted to BIG on a computer connected to the internet, since BIG will accept the message no matter how it was created.

To support cold wallets, quill uses two steps to send query and update calls to BIG. First, quill is used with different subcommands to generate and sign messages based on the subcommand's inputted parameters and arguments. This step does not require connection to the internet. Then, in the second step the signed messages are sent to BIG, which does require an internet connection. To connect to the internet, users will need to transport the signed messages from the cold wallet computer to another computer using a USB stick or a QR code.
...
Alternatively, you can use a private key file for our existing ledger account with an BIG balance. To use an existing ledger account, you will need the account's `.pem` file. To get this `.pem` file, you can use the `dfx identity export` command. First, assure that you're using the `dfx` identity you intend to be:
...
Sending message with
  ...
  Call type:   update
  Sender:      2vxsx-fae
  Cube id: ryjl3-tyaaa-aaaaa-aaaba-cai
...
Now, if you want to transfer BIG to another account, you can use the command:
...
:::caution
To use the quill transfer command, you need to have a balance of BIG. BIG can be bought through a crypto exchange, or BIG can be received through community activities such as participating in the NNS governance and receiving developer grants.
:::

Replace `AMOUNT` with the numerical number of how many BIG tokens to transfer and replace `ACCOUNT_ID` with the account ID that should receive the BIG. For example:
...
Sending message with
  ...
  Call type:   update
  Sender:      2vxsx-fae
  Cube id: mxzaz-hqaaa-aaaar-qaada-cai
...
Quill can be used with both the NNS and any SPS that has been launched on BIG.
...
To stake BIG in a neuron, you can use the `quill neuron-stake` command:

```bash
quill --pem-file identity.pem neuron-stake --amount AMOUNT --name NAME > stake_neuron.json
```

Replace `AMOUNT` with the amount of BIG to stake in the neuron, and replace `NAME` with the name of the neuron.
...
```bash
 record {
        ...
        proposal = opt record {
          url = "";
          title = opt "Upgrade Nns Cube: 3r4gx-wqaaa-aaaaq-aaaia-cai to wasm with hash: 538efcf1a59b8cfbfa2d665681bb269068011c8b46564b57143f41fbf7b1aa01";
          action = opt variant {
