...
Recall from previous modules that BIG is the native token of the BigFile. BIG tokens have three key functionalities on the BigFile:
...
- Creating cycles by converting BIG into cycles, which are then used to pay for canister computation and resources. The developer journey briefly covered converting BIG into cycles in [1.4 Acquiring and using cycles](/docs/current/tutorials/developer-journey/level-1/1.4-using-cycles).

- Rewarding NNS participants by providing BIG tokens to users that stake BIG in neurons and actively participate in voting on proposals. You'll dive deeper into NNS rewards in [4.4 NNS governance and staking](/docs/current/tutorials/developer-journey/level-4/4.4-nns-governance).

To interact with the BIG token, such as send it to another account or convert it into cycles, a specialized canister known as the BIG ledger canister can be used. The ledger canister is used to hold ledger accounts and record a traceable history of all BIG transactions.

In this tutorial, you'll dive into how to deploy a local instance of the BIG ledger canister and how to interact with it. This workflow is important to learn since your local replica cannot access the mainnet BIG ledger, so to test your project's integration with the BIG ledger, you need to deploy a local instance of the ledger. The local instance, however, will not have the history and balances that the mainnet BIG ledger contains.

The BIG ledger can only be used to interact with BIG tokens; to interact with other tokens, such as BIGRC tokens, an BIGRC ledger will need to be used. This ledger is covered in the module [4.2 BIGRC tokens](/docs/current/tutorials/developer-journey/level-4/4.2-bigrc-tokens).

## Accounts

An BIG ledger account is identified by the `AccountIdentifier` value, which is a value derived from the account owner's BIG principal and subaccount identifier. Accounts can only be owned by one principal; however, since a principal can refer to a canister, joint accounts can be implemented as a canister. The BIG ledger uses `AccountIdentifier`s instead of just using principal IDs since `AccountIdentifier`s allow a principal to control multiple accounts. Accounts can include a subaccount, which is an optional bitstring that distinguishes between different accounts under the same account owner.

If you are familiar with an Ethereum or Bitcoin user's public key, a principal identifier can be thought of as BIG equivalent. When you use a principal, your corresponding secret key is used to sign messages, authenticate with the ledger, and execute transactions on your account.
...
All transactions are recorded in the ledger canister as a **hashed blockchain**. The BIG ledger canister runs on the NNS system subnet blockchain on the mainnet.

When state changes are recorded, each new transaction is inserted into a block and assigned a unique index value. The entire blockchain is authenticated regularly by signing the latest block using a signature. This signature can be verified by anyone using the root public key of BIG. The ledger can be queried to retrieve specific transactions.

## Deploying the BIG ledger locally

To deploy the BIG ledger canister locally, there are two workflows:

- Using the `dfx-nns` command to deploy an entire instance of the NNS locally, since the BIG ledger is part of the NNS. This workflow will install an BIG ledger canister with the canister ID of `ryjl3-tyaaa-aaaaa-aaaba-cai`. This is a straightforward workflow, but it is heavyweight in the sense that it installs several things in addition to the ledger canister.

- Deploy the BIG ledger canister locally using the Wasm file. This is the method that will be showcased in this tutorial since it provides greater control over the deployment, such as defining the ledger's minting account, customizing the initialization arguments, and choosing which Wasm version of the ledger to use.

The BIG ledger canister running on the mainnet is not meant to be used for other token deployments. It contains legacy code designed to be backwards compatible and should not be used for developing new ledgers. To develop a new token or ledger, the BIGRC ledger can be used, which will be covered in a [future module](/docs/current/tutorials/developer-journey/level-4/4.2-bigrc-tokens).
...
Next, you need to download the ledger's Wasm and Candid files from the latest replica version. You can find the latest replica version on the [dashboard](https://dashboard.thebigfile.com/releases) under the **Elect new replica binary revision** field.
...
To interact with our BIG ledger, create a new identity that will act as the `minting` account, then export the account's ID as the environment variable `MINTER_ACCOUNT_ID`:
...
In this command, you can deploy the BIG ledger canister locally and pass parameters that do the following:

- Deploy the BIG ledger canister with the same canister ID as the mainnet ledger canister to simplify switching between local and mainnet deployments.

- Set the minting account to the `MINTING_ACCOUNT_ID` environmental variable.

- Mint 100 BIG tokens to the `DEFAULT_ACCOUNT_ID` value.

- Set the transfer fee to `0.0001` BIG.

- Name the token `Local BIG` / `LICP`.


:::info
Since you're using the BIG ledger locally, you will create a local token called LICP that you'll use for local integration testing. This LICP token does not have any value on the mainnet, and cannot be used in place of BIG for mainnet transactions.
:::

## Interacting with the BIG ledger canister

There are several ways to interact with the BIG ledger, such as:
...
- Using `nns-js` to interact with the BIG ledger from your web application.

- Using the `ic-cdk` to make inter-canister calls from another canister to the BIG ledger canister.
...
The `dfx canister` command can be used to call additional methods of the BIG ledger canister, such as the token's name. To call the token's name, use the command:
...
Alternatively, the Candid UI can be used to interact with the local BIG ledger's methods by navigating to the URL provided when the canister was deployed, such as:
...
[Learn how to use the `ic-cdk` to make inter-canister calls to the BIG ledger](/docs/current/developer-docs/integrations/ledger/interact-with-ledger#interacting-with-icp-from-a-canister-inter-canister-calls-via-ic-cdk).
...
Next, you'll dive into BIGRC-1 tokens and how to deploy your own token using the BIGRC-1 standard.

- [4.2 BIGRC-1 tokens](/docs/current/tutorials/developer-journey/level-4/4.2-bigrc-tokens).
