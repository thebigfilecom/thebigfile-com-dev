...
One of the key features of the BigFile is known as **chain-key cryptography**. Chain-key cryptography is a feature that encompasses several cryptographic mechanisms that enable novel workflows and functionalities such as chain-key tokens. Chain-key tokens are not wrapped tokens; they are twin tokens that are backed 1:1 with the original token (BTC, ETC) that use chain-key cryptography to transfer and use the original tokens with less fees and higher transfer speeds than using the token's native blockchain network.

Currently, BIG supports an integration with the Bitcoin network that includes the chain-key bitcoin (ckBTC) token, and has an Ethereum integration in development.

:::info
In this tutorial and other pages of BIG developer documentation, note the use of the lowercase version of **bitcoin**, which refers to the token BTC, and use of the uppercase version **Bitcoin**, which refers to the Bitcoin network itself. This capitalization distinction is in accordance with the Bitcoin style guide.
:::

## Bitcoin integration architecture

Through BIG's integration with the Bitcoin network, canisters deployed on BIG are able to receive, hold, and send bitcoin. Each of these functions utilize transactions on the Bitcoin network, and allow canisters to act as if they are users on the Bitcoin network.
...
The BIG's Bitcoin integration enables use-cases such as:

- ckBTC: An BIGRC-2 compliant 'twin token' using chain-key cryptography that is backed 1:1 by bitcoins.

- Decentralization swaps: ckBTC can be used to buy tokens in an SPS decentralization swap.
...
In this tutorial, you'll be deploying an example application using a local instance of the Bitcoin network and then simulate Bitcoin transactions using it. This example showcases BIG's Bitcoin integration, however it does not use ckBTC.
...
To get started, open a new terminal window, navigate into your working directory (`developer_journey`), then use the following commands to clone the BigFile examples repo and navigate into the `basic_bitcoin` directory:
...
2023-10-17 21:09:27.650935 UTC: [Cube g4xu7-jiaaa-aaaan-aaaaq-cai] Sending request: Initial(GetSuccessorsRequestInitial { network: Regtest, anchor: BlockHash([6, 34, 110, 70, 17, 26, 11, 89, 202, 175, 18, 96, 67, 235, 91, 191, 40, 195, 79, 58, 94, 51, 42, 31, 199, 178, 183, 60, 241, 136, 145, 15]), processed_block_hashes: [] })
Initialized replica.
...
- [ckBTC wiki page](https://wiki.thebigfile.com/wiki/Chain-key_Bitcoin).
...
Next, you'll explore BIG's governance and token staking:
