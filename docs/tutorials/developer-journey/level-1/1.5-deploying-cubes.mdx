...
Now that you have some cycles to pay for your cube's resources, you can deploy your dapp on the mainnet. When a cube is deployed to the mainnet, it's code will be hosted on the nodes that make up one of the mainnet's subnets. The cube will be hosted on these nodes until it is manually uninstalled by the developer, or until it runs out of cycles and is uninstalled automatically.

Once a dapp is deployed on the mainnet, it can be accessed from the cube's public URL by end-users and can communicate with other cubes that are deployed on the mainnet.
...
This command doesn't include a cube name, meaning it will automatically deploy **all** cubes that are specified in the `dfx.json` file. You can specify a single cube to be deployed by including the cube's name in the command, such as:

```bash
dfx deploy poll_backend --network ic
```

You want to deploy both the `poll_backend` and `poll_frontend` cubes since they work together to create your poll dapp, so the `dfx deploy --network ic` command should be used for this tutorial.
...
If you manually converted BIG tokens into cycles, and your cycles balance doesn't contain enough cycles to complete this deployment, you can convert more BIG into cycles with the command:
...
This command converts an additional 1.005 BIG tokens to cycles.

If you used the free cycles coupon, this step is not applicable.

Now, let's use your dapp! To access the dapp's frontend, first you need to get the cube's URL. To get this, run the command:

```bash
dfx canister id poll_frontend --network ic
```

This command will return the cube's URL, which should look like this:
...
Now that you have cubes running on the mainnet, the next steps are to learn how you can manage these cubes, such as how to upgrade them or delete them.

- [1.6 Managing cubes](/docs/current/tutorials/developer-journey/level-1/1.6-managing-cubes)
