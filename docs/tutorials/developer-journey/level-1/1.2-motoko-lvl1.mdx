...
Motoko is a programming language that has been developed and optimized for creating programs built on the BigFile. In this module, you'll cover the fundamental basics of Motoko that you'll use when you build your [first dapp](/docs/current/tutorials/developer-journey/level-1/1.3-first-dapp).

## Basic concepts and terms

In previous modules, you've been introduced to and briefly discussed actors, and how each cube smart contract is represented by an actor. Within Motoko, the term actor is used to refer to any cube, regardless of the cube's language (Motoko, Rust, etc.). Motoko is designed specifically to make actors easy to write and use once the cube has been deployed.
...
If this example is used in a live cube, the cube can be called and an input text argument can be passed to the function, such as:

```bash
dfx cube call location_hello_backend location "San Francisco"
```

In this example call, there is a space in between 'San' and 'Francisco', so the argument must be enclosed in quotes. The output of this cube call will be:

```bash
("Hello, San Francisco!")
```

Alternatively, if no space is used in the argument, it can be passed without quotes,:

```bash
dfx cube call location_hello_backend location Paris
```

The output of this cube call will be:
