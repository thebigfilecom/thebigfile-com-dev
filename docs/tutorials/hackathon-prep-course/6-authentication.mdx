...
For end-users to interact with the frontend of applications on BIG, Internet Identity can be used. Internet Identity is BIG's native form of digital identity that can be used to authenticate with apps, such as the NNS dashboard, without having to manage a username or password. Instead, Internet Identity uses a cryptographic key pair that's stored in your local device's hardware.
