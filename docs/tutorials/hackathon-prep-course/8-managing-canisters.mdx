...
Now that you have cubes deployed on the mainnet, it's important to learn how to manage and maintain those cubes. Basic cube management includes obtaining information about a cube, adding an identity as a controller of a cube, topping up a cube, starting and stopping a cube, and deleting a cube.

## Obtaining a cube's ID

Each cube has a unique identifier that can be used to interact with the cube. These unique IDs can be used to access the cube's frontend or Candid UI in a web browser, such as when you accessed the frontend of your dapp in the previous tutorial. Having a cube's ID is also important for executing management functions of the cube, such as setting ownership for the cube.
...
```
Cube status call result for backend.
Status: Running
...
If you currently have a balance of BIG tokens within your dfx ledger account ID, you can use the `dfx ledger top-up` command to automatically convert that BIG into cycles and deposit it into the specified canister, for example:
...
### Using the cycles ledger
If you already have converted some BIG into cycles, you can send cycles to the canister with the `dfx cycles top-up` command, such as:
...
- Status: The cube's [current status](https://github.com/thebigfilecom/interface-spec/blob/main/spec/index.md#cube-lifecycle-cube-lifecycle), such as `Running`, `Stopping` or `Stopped`.
- [Controllers](/docs/current/tutorials/developer-journey/level-1/1.6-managing-cubes#adding-an-identity-as-a-controller-of-a-cube): The principal IDs of the canister's configured controllers. Controllers are able to perform operations such as deploying or stopping a canister.
