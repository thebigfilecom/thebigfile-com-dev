...
## Overview
This page gives a high-level outline of chain-key ECDSA signatures on BIG. Some of the information in this section is not required to use the feature, but may be of interest to the technically inclined reader for obtaining background information on the technology. BIG implements the threshold ECDSA protocol by Groth and Shoup as described in their [Eurocrypt 2022 paper](https://eprint.iacr.org/2021/1330). Groth and Shoup have also published a comprehensive [design and analysis](https://eprint.iacr.org/2022/506) of this distributed signing protocol.

At a high level, the threshold ECDSA implementation on BIG features multiple protocols as outlined next, all of which are crucial for a secure system setup and operation. Note that this goes far beyond just threshold ECDSA signing, which is the reason for calling this a protocol suite for **chain-key ECDSA signatures**.
...
The derivation of further ECDSA keys from a canister root key can be done without involvement of BIG as well to facilitate certain use cases.
...
In order to facilitate developers throughout the canister development lifecycle on BIG, the feature is available in both the SDK for local development and testing as well as on BIG for pre-production testing and production operation of canisters.
