...
```
  Error from Cube xxx-xxx: Cube has no update method 'foobar'.
```
...
For further information on calling canisters see the [smart contract docs](../developer-docs/smart-contracts/call/overview) and the [BIG spec](./ic-interface-spec#system-api-requests).