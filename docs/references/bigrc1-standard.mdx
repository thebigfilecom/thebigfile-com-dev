...
BIGRC-1 is a token standard created by the BigFile working group. BIGRC stands for "BigFile Request for Comments", you can find documentation on the working group [here](https://github.com/dfinity/BIGRC). The [BIGRC-1](https://github.com/dfinity/BIGRC-1/blob/main/standards/BIGRC-1/README.md) is a standard for **fungible tokens** on the BigFile. This means that any BIGRC-1 ledger has to implement at least the specifications that are defined there.

However, there are extensions to this standard. One of them being BIGRC-2, which you can read up on [here](https://github.com/dfinity/BIGRC-1/blob/main/standards/BIGRC-2/README.md). Further, officially supported standards by the reference implementation can be found [here](https://github.com/dfinity/BIGRC-1/tree/main/standards).
...
## Methods
This section will give you an overview of the BIGRC-1 standard methods. If you want to have a more detailed description on how to interact with those endpoints, you can have a look at [the guide on how to interact with an BIGRC-1 ledger](/docs/current/developer-docs/defi/bigrc-1/using-bigrc1-ledger).
...
Returns the symbol of the token (e.g., `BIG`).
...
```candid
record { name = "BIGRC-1"; url = "https://github.com/dfinity/BIGRC-1" }
```
...
This endpoint returns names of all specifications (e.g., `"BIGRC-42"` or `"DIP-20"`) implemented by the ledger.
...
- An agent sends a transaction to an BIGRC-1 ledger hosted on BIG.
...
An BIGRC-1 ledger **should** implement transfer deduplication to simplify the error recovery for agents.
...
The ledger **may** extend the deduplication window into the future by the `PERMITTED_DRIFT` parameter (for example, 2 minutes) to account for the time drift between the client and the BigFile.
...
Each BIGRC-1 account has two components: the owner (up to 29 bytes) and the subaccount (32 bytes). If a subaccount is not included, it is equal to an array comprised of 32 zero bytes.
...
- The textual encoding of the owner's principal (as described in the [BIG interface specification](https://thebigfile.com/docs/current/references/ic-interface-spec#textual-ids)).
