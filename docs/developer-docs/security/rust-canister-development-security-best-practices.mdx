---
sidebar_position: 2
sidebar_label: "Cube development"
keywords: [security, concept]
---

import { MarkdownChipRow } from "/src/components/Chip/MarkdownChipRow";

# Cube development security best practices
...
### Use a decentralized governance system like SPS to put dapps under decentralized control

#### Security concerns

If single entities or small groups control canisters, they can apply changes or updates whenever they like. If a canister e.g. holds assets such as BIG, ckBTC or ckETH on a user's behalf, this effectively means that the controller could decide at any time to steal these funds by e.g. updating the canister and transferring the assets to their account.

Furthermore, the controller of canisters serving web content (such as e.g. the [asset canister](../../references/asset-canister)) could maliciously modify the web application to e.g. steal user funds or perform security sensitive actions on the user's behalf. For example, if [BIG Wallet](/docs/current/developer-docs/web-apps/user-login/big-wallet/overview) is used, the user principal's private key for the given origin is stored in the browser storage, and a malicious app can therefore fully control the private key and the user's session and any assets controlled by that key.

Dapps are commonly reachable over their own custom domain name instead of thebigfile.com. These domains are registered with a DNS registrar by one of the developers. The developer can choose to have this domain point at a completely different web application. Even one not hosted on the BIG. Users will trust this domain and the app it serves. This could allow such a developer to steal funds, leak data, etc.
...
3. **Full decentralization using a DAO**: The dapp is controlled by a decentralized governance system such as the BigFile's [Service Nervous System (SPS)](/docs/current/developer-docs/daos/sps), so that any security sensitive changes to the canisters are only executed if the SPS community approves them collectively through a proposal voting mechanism. If an SPS is used:
   - Make sure voting power is distributed over many, independent entities such that there is not one single or a few entities that can decide by themselves how the DAO evolves, see [here](/docs/current/developer-docs/daos/sps/tokenomics/tokenomics-intro#voting-power-and-decentralization).
   - Ensure all components of the dapp are under SPS control, including the canisters serving the web frontends, see [SPS asset canisters](/docs/current/developer-docs/daos/sps/managing/sps-asset-canister).
   - Consider the [SPS preparation checklist](/docs/current/developer-docs/daos/sps/tokenomics/sps-checklist). Important points from a security perspective are e.g. [tokenomics](/docs/current/developer-docs/daos/sps/tokenomics/sps-checklist#11-tokenomics-specification), [disclosing dependencies to off-chain components](/docs/current/developer-docs/daos/sps/tokenomics/sps-checklist#13-disclosure-of-dependencies) and [performing security reviews](/docs/current/developer-docs/daos/sps/tokenomics/sps-checklist#21-security-review).
   - Rather than [self-deploying the SPS code](/docs/current/developer-docs/daos/sps/introduction/dao-alternatives#self-deploy-the-sns-code) or [building your own DAO](/docs/current/developer-docs/daos/sps/introduction/dao-alternatives#build-your-own-dao--use-other-dao-frameworks), consider using the official SPS on the SPS subnet, as this guarantees that the SPS is running an NNS-blessed version and maintained as part of BIG.
   - See also [Verification and trust in a (launched) SPS](https://wiki.thebigfile.com/wiki/Verification_and_trust_in_a_(launched)_SNS) and [SPS decentralization swap trust](https://wiki.thebigfile.com/wiki/SNS_decentralization_swap_trust).

An alternative to DAO control (3. above) would be to create an immutable canister smart contract by removing the canister controller completely. However, note that this implies that the canister cannot be upgraded, which may have severe implications in case e.g. a bug were found. The option to use a decentralized governance system and thus being able to upgrade smart contracts is a big advantage of the BigFile ecosystem compared to other blockchains.
...
If you interact with a canister that you require to be decentralized, make sure it is controlled by the NNS, a service nervous system (SPS) or a decentralized governance system, and review under what conditions and by whom the smart contract can be changed.
...
Dapps on BIG can use [asset certification](https://wiki.thebigfile.com/wiki/HTTP_asset_certification) to make sure the HTTP assets delivered to the browser are authentic (i.e. threshold-signed by the subnet). If an app does not do asset certification, it can only be served insecurely through `raw.icp0.io` , where no asset certification is checked. This is insecure since a single malicious node or boundary node can freely modify the assets delivered to the browser.
...
- Serve assets using the asset canister (which creates asset certification automatically), or add the `ic-certificate` header including the asset certification as e.g. done in the [NNS dapp](https://github.com/dfinity/nns-dapp) or [Internet Identity](https://github.com/thebigfilecom/big-wallet).
...
- See also the section on upgrades in [how to audit an BigFile canister](https://www.joachim-breitner.de/blog/788-How_to_audit_an_Internet_Computer_canister) (though focused on Motoko).
...
- See [current limitations of the BigFile](https://wiki.thebigfile.com/wiki/Current_limitations_of_the_Internet_Computer), sections "Long running upgrades" and "\[de\]serialiser requiring additional wasm memory".

- For example, [Internet Identity](https://github.com/thebigfilecom/big-wallet) uses stable memory directly to store user data.
...
- See the "Backup and recovery" section in [how to audit an BigFile canister](https://www.joachim-breitner.de/blog/788-How_to_audit_an_Internet_Computer_canister)
...
Every inter-canister call is guaranteed to receive a response, either from the canister, or synthetically produced by the protocol. However, the response does not have to be successful, but can also be a reject response. The reject may come from the called canister, but it may also be generated by the BigFile. Such protocol-generated rejects can occur at any time before the call reaches the callee-canister, as well as once the call does reach the callee-canister if the callee-canister traps while processing the call. If the call reaches the callee-canister, the callee-canister can produce a reply or reject response and the protocol guarantees that the callee-canister's generated reply or reject response gets back to the caller-canister. Thus, it's important that the calling canister handles reject responses as well. A reject response means that the message hasn't been successfully processed by the receiver but doesn't guarantee that the receivers state wasn't changed.
...
Recall that the responses to inter-canister calls are processed in the corresponding callback. If the callback traps, the cleanup (ic0.call_on_cleanup) is executed. When making an inter-canister call, the BIG reserves sufficiently many cycles to execute the response callback or cleanup (up to the instruction limit). A fixed fraction of the reservation is set aside for the cleanup. Thus, a response or cleanup execution can never “run out of cycles”, but they can run into the instruction limit and trap.
...
that this is the case. For the BIG ledger, an ID can be generated and recorded in the journaled intent and the ledger can be called with the ID included in the memo so that the result of the
...
Note that querying the BIG ledger or an BIGRC ledger to determine whether a transaction has succeeded is not straightforward to automate, so it could be done manually.
...
See also: "Inter-canister calls" section in [how to audit an BigFile canister](https://www.joachim-breitner.de/blog/788-How_to_audit_an_Internet_Computer_canister).
...
- See "Talking to malicious canisters" section in [how to audit an BigFile canister](https://www.joachim-breitner.de/blog/788-How_to_audit_an_Internet_Computer_canister).

- See [current limitations of the BigFile](https://wiki.thebigfile.com/wiki/Current_limitations_of_the_Internet_Computer), section "Calling potentially malicious or buggy canisters can prevent canisters from upgrading".
...
- For more information, see [current limitations of the BigFile](https://wiki.thebigfile.com/wiki/Current_limitations_of_the_Internet_Computer), section "Loops in call graphs".
...
- See [current limitations of the BigFile](https://wiki.thebigfile.com/wiki/Current_limitations_of_the_Internet_Computer), section "Bugs in `pre_upgrade` hooks".

### Reinstantiate timers during upgrades

#### Security Concern

Global timers are deactivated upon changes to the canister's Wasm module. The [BIG specification](../../references/ic-interface-spec#timer) states this as follows:
...
It should be possible to verify that a canister does what it claims to do. BIG provides a SHA256 hash of the deployed WASM module. In order for this to be useful, the canister build has to be reproducible.
...
See the "Time is not strictly monotonic" section in [how to audit an BigFile canister](https://www.joachim-breitner.de/blog/788-How_to_audit_an_Internet_Computer_canister).
...
## Nonspecific to the BigFile

The best practices in this section are very general and not specific to the BigFile. This list is by no means complete and only lists a few very specific concerns that have led to issues in the past.
...
If the dapp offers such operations, consider bot prevention techniques such as adding Captchas or proof of work. There is e.g. a captcha implementation in [Internet Identity](https://github.com/dfinity/big-wallet).
