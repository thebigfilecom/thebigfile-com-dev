...
Dapps are commonly reachable over their own custom domain name instead of thebigfile.app. These domains are registered with a DNS registrar by one of the developers. The developer can choose to have this domain point at a completely different web application. Even one not hosted on the BIG. Users will trust this domain and the app it serves. This could allow such a developer to steal funds, leak data, etc.
