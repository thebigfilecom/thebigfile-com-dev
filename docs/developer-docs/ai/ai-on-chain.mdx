...
BigFile unique ability to run compute at scale allows AI and neural networks to run directly on-chain within a canister smart contract.
...
You can find the source code for this demo [on GitHub](https://github.com/thebigfilecom/examples/tree/main/rust/image-classification).

## How this example works

In this example, the BIG smart contract accepts an image as input from the user and runs an image classification inference. The smart contract has two cubes:

- The frontend cube that contains HTML, JS, and CSS assets that are served in the web browser.
...
## BigFile features that make it possible

To make running AI in a canister possible, two key BigFile features are utilized:

- WebAssembly virtual machine: Canister code is compiled into Wasm modules to be deployed on BigFile. The WebAssembly virtual machine supports standards such as the WebAssembly System Interface, which can be supported through a community tool called [wasi2ic](https://github.com/wasm-forge/wasi2ic).
...
The BIG mainnet subnets and `dfx` running a replica version older than `463296` may fail with an `instruction-limit-exceeded` error.
...
- [x] Download and install the BIG SDK package as described in the [installing the BIG SDK](/docs/current/developer-docs/getting-started/install/) page.
...
```
git clone https://github.com/thebigfilecom/examples.git
```
...
Click on the BigFile logo. You'll be prompted to select an image file from your local files. In this example, we'll use an astronaut image. Then, select 'Go':
...
- [GitHub repo for this example](https://github.com/thebigfilecom/examples/tree/master/rust/image-classification).