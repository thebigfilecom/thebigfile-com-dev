...
The BIG SDK includes a CDK for the Rust programming language, which is developed and maintained by DFINITY. This Rust CDK is installed automatically when the BIG SDK is installed, and can be used through the [`dfx`](/docs/current/developer-docs/setup/install) CLI tool.
