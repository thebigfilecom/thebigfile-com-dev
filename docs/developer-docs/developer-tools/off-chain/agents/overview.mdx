...
In the BigFile ecosystem, a library that is used to make calls to the BIG public interface is called an **agent**. An agent has a few key responsibilities, which make it convenient to work with in your language of choice.

If you have a canister running, either on your local machine or live on the BigFile, you will have two main ways to interact with your canister smart contract.
...
BIG supports two types of calls - `queries` and `updates`. Queries are fast and cannot change state. Updates go through consensus, and will take around 2-4 seconds to complete.
...
- JavaScript / TypeScript
  - [JavaScript/TypeScript agent by BigFile](/docs/current/developer-docs/developer-tools/off-chain/agents/javascript-agent)
- Rust
  - [Rust agent by BigFile](/docs/current/developer-docs/developer-tools/off-chain/agents/rust-agent)
...
A `call` to the BigFile can take two common forms - an `update` or a `query`. The **agent** submits a `POST` request to `/api/v2/canister/<effective_canister_id>/call`, and includes the following components:
...
For update requests, the agent also hashes the rest of the fields, and passes it in as a unique `request_id`. That `request_id` is used for polling while BIG reaches consensus on the update.
...
Once the data has been returned from BIG, the **agent** takes the certificate from the payload and verifies it. The certificate can be verified as genuine using the public `rootKey` of the NNS subnet. The network will respond with a CBOR-encoded buffer, which the **agent** can then decode, and transform into a useful structure using semantic language-specific types. For example, if the type returned from the canister is `text`, that will get turned into a JavaScript `string`, and so on.

### Managing authentication

Calls to the BigFile always need to have a cryptographic identity attached. That identity will either be **anonymous** or **authenticated**, using a cryptographic signature. Since identities are required, canisters can use the identity attached to a call to decide how to respond to that call. This enables contracts to use those identities for other purposes.

#### Accepted identities

BIG accepts calls using the following types of signatures in identities:
