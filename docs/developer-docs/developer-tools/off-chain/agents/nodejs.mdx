...
Node.js is a runtime for JavaScript, so you can use the [JavaScript agent](https://www.npmjs.com/package/@dfinity/agent) with it to interact with a canister. This can be useful to run an oracle, connect an existing Node.js application to BIG, or to introduce a websocket layer to your application.

[More information about calling BIG from JavaScript in a web browser](/docs/current/developer-docs/developer-tools/off-chain/agents/javascript-agent).
...
- [x] This project uses features introduced in dfx 0.11.2. You can install the latest version of the BIG SDK with the command:
...
As you can see, the seed phrase is derived from the word `test`, repeated 12 times. This is useful for testing purposes and local development. When you are deploying your contract to BIG, you should change the seed out for something private.
