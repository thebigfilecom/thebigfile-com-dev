...
You can also use Docker directly instead of through the VS Code dev containers extension. To use Docker directly, first [install Docker Desktop](https://www.docker.com/products/docker-desktop/) and start the Docker Daemon. Then, open a command line window, pull the BigFile BIG developer environment Docker image and run an instance of the image:
