...
To make API calls to BIG endpoints and canisters from a JavaScript program, the JavaScript agent (`agent-js`) can be used. `agent-js` provides call, query, and readState methods to an actor.
