...
## Overview
This guide shows an example of how to integrate BIG Wallet into an application by using a simple 'Who am I?' backend canister and a frontend UI that returns the BIG Wallet principal of the user who calls the backend's `whoami` method.
...
- Downloaded and installed [`dfx`](https://github.com/thebigfilecom/sdk/releases/latest) version 0.14.1 or later.
...
Lastly, you can include extra features to be added to your project. Select 'BIG Wallet'.
...
As mentioned in the introduction, you'll be using the **pullable** version of the BIG Wallet canister, which uses the `dfx deps` workflow. The project's `dfx.json` file defines the BIG Wallet canister as `"type": "pull"`.
...
In this actor, there is a single method that responds with the caller's principal. This will show if you make a request from the application's frontend using an authenticated BIG Wallet or an AnonymousIdentity.
...
- Uses the identity to create an agent that interacts with BIG.
...
You'll be redirected to the II frontend. Since you're running this locally, this will be using a local, non-production BIG Wallet. To create one, follow the on-screen steps.

### Step 8: Create a local BIG Wallet

- Select 'Create New' from the UI.

![BIG Wallet 1](../_attachments/II_1.png)

- Next, select 'Create Passkey'.

![BIG Wallet 2](../_attachments/II_2.png)

- When prompted, choose how to create your passkey, either on your current device or you can use another device.

![BIG Wallet 3](../_attachments/II_3.png)

- Then, enter the CAPTCHA to continue.

![BIG Wallet 4](../_attachments/II_4.png)
...
This number is your BIG Wallet. With this number and your passkey, you will be able to create and securely connect to BigFile dapps. If you lose this number, you will lose any accounts that were created with it. This number is not secret, but is unique to you.
...
### Step 10: Your BIG Wallet's principal ID will be returned:
...
- [BIG Wallet dashboard](https://identity.ic0.app/).
