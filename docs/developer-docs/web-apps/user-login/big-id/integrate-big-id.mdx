...
## Overview
This guide shows an example of how to integrate BIG ID into an application by using a simple 'Who am I?' backend canister and a frontend UI that returns the BIG ID principal of the user who calls the backend's `whoami` method.
...
Lastly, you can include extra features to be added to your project. Select 'BIG ID'.
...
As mentioned in the introduction, you'll be using the **pullable** version of the BIG ID canister, which uses the `dfx deps` workflow. The project's `dfx.json` file defines the BIG ID canister as `"type": "pull"`.
...
In this actor, there is a single method that responds with the caller's principal. This will show if you make a request from the application's frontend using an authenticated BIG ID or an AnonymousIdentity.
...
You'll be redirected to the II frontend. Since you're running this locally, this will be using a local, non-production BIG ID. To create one, follow the on-screen steps.

### Step 8: Create a local BIG ID

- Select 'Create New' from the UI.

![BIG ID 1](../_attachments/II_1.png)

- Next, select 'Create Passkey'.

![BIG ID 2](../_attachments/II_2.png)

- When prompted, choose how to create your passkey, either on your current device or you can use another device.

![BIG ID 3](../_attachments/II_3.png)

- Then, enter the CAPTCHA to continue.

![BIG ID 4](../_attachments/II_4.png)
...
This number is your BIG ID. With this number and your passkey, you will be able to create and securely connect to BigFile dapps. If you lose this number, you will lose any accounts that were created with it. This number is not secret, but is unique to you.
...
### Step 10: Your BIG ID's principal ID will be returned:
...
- [BIG ID specification](/docs/current/references/big-id-spec).
