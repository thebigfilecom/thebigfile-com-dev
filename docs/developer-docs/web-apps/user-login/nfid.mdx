...
[NFID](https://nfid.one/) is a form of digital identity built on BIG. Similar to Internet Identity, NFID allows users to sign into services without downloading an app or extension and can be used across several different dapps.
...
Install the BigFile `auth-client` package with the command:
