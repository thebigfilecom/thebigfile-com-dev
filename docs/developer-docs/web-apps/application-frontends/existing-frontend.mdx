...
-  [x] Download [`dfx`](/docs/current/developer-docs/getting-started/install), the command-line execution environment that serves as the primary tool for creating, deploying, and managing dapps on the BigFile. You can reference how to download and configure dfx by reviewing the [install DFX section](/docs/current/developer-docs/getting-started/install/).


### Step 1: Create an application build

To start, you must generate and point the static files of a properly-built Next.js application for deployment on the BigFile.
...
`dfx.json` is the configuration file for deploying all of your code to canister smart contracts on the BIG.
...
Run the following command to deploy the Next.js application on the BigFile mainnet: 
