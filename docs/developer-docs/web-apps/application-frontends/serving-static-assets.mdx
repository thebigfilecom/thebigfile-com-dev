...
When a dapp is deployed to BIG and has a frontend configured, the frontend assets will be displayed via a URL that contains the canister's ID. For local deployments, canisters are accessible through local URLs such as `http://127.0.0.1:4943/?canisterId=<canister-id>`. The port `4943` is the default local deployment port, though this can be changed via the `dfx` configuration or using a `dfx` flag. [Learn more about custom local networks](/docs/current/developer-docs/developer-tools/cli-tools/advanced-dfx/networks-json).
...
<html> 
<head> 
<title>Static BIG webpage</title> 
<meta charset="UTF-8"> 
<meta name="description" content="This is a static webpage hosted on BIG using a frontend canister."> 
</head> 
<body> 
<h1>Welcome to my BIG-hosted webpage</h1> 
<p>This showcases how static assets can be hosted within an BIG canister.</p>
