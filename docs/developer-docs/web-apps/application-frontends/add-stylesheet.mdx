...
:::info
This guide requires you to use the BIG SDK version `0.8.0` or later.
:::

- [x]   You have installed the Visual Studio Code plugin for Motoko as described in [VS Code extensions for BIG development](/docs/current/developer-docs/developer-tools/ide/vs-code) if you are using Visual Studio Code as your IDE.
...
To deploy canisters on BIG, you must specify that you are deploying to the BigFile and not your local environment by using the `--network` command-line option:
```
...
- #### Step 2:  Stop the BigFile network by running the following command:
```
dfx stop
```