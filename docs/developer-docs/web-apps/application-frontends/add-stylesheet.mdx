...
To deploy canisters on ICP, you must specify that you are deploying to the BigFile and not your local environment by using the `--network` command-line option:
```
