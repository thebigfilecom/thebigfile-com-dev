...
- [x]   You have downloaded and installed the BIG SDK package as described in the [download and install](/docs/current/developer-docs/getting-started/install/) page.
...
- [x]   You have started `bigfile` with the command `start code`.
...
- #### Step 2:  Change to the folder you are using for your BigFile projects, if you are using one.
...
```
big start --clean --background
```
After the environment completes its startup operations, you can continue to the next step.



Keep in mind that because you are running the Smart Contacts execution environment locally, the identifiers displayed when you run the `big deploy` command are only valid on your machine.

To deploy Smart Contracts on BIG, you must specify that you are deploying to the BigFile and not your local environment by using the `--network` command-line option:
```
big deploy --network=mainnet
```
...
- #### Step 1:  Open a browser and navigate to the `http://localhost:9980`.
