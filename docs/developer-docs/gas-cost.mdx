...
- [The BIG reverse gas model](#the-icp-reverse-gas-model)
    - [How do BIG costs compare to the EVM gas model?](#how-do-icp-costs-compare-to-the-evm-gas-model)
- [Cube calls](#canister-calls)
    - [Cube creation](#canister-creation)
    - [Messaging](#messaging)
    - [Execution](#execution)
    - [Storage](#storage)
...
In order to encourage long-term usage and discourage spiky usage patterns, the BigFile uses a *resource reservation mechanism* that was adopted by the community in [NNS proposal 12604](https://dashboard.thebigfile.com/proposal/126094).
...
Getting cycles back from a cube
   - To withdraw cycles from a cube, the [cube must be deleted](/docs/current/tutorials/developer-journey/level-1/1.6-managing-cubes#getting-cycles-back-from-a-cube).
