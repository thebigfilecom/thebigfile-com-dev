...
The [asset canister](https://github.com/dfinity/sdk/tree/master/src/canisters/frontend/ic-frontend-canister) provides users with a way to store and retrieve static assets from a canister deployed on BIG. Generally, asset canisters are used to serve HTML, CSS, or JavaScript assets, which are typically part of a dapp's frontend. For this reason, the asset canister is also referred to as the frontend canister. For purposes of this guide, it will be referred to as the asset canister.

In the context of the SPS, a dapp's associated asset canister serves the frontend assets related to the dapp and may also include a frontend to the SPS DAO, e.g., through which users can vote on governance proposals.

The contents of the asset canister can be configured prior to the launch of the SPS. If configured correctly, any changes afterwards must be made by a principal with the `Prepare` permission. Principals with this `Prepare` permission can make a batch of changes to the asset canister and then 'lock' those changes. To have those changes applied, a proposal must be submitted. Anyone can submit the proposal for the 'locked' changes. Once changes are proposed, they can be voted on by the SPS DAO. If approved, the SPS governance canister is the only one that can commit the approved changes. This configuration assures that changes to the asset canister are only made by approved proposals. These changes are referred to as 'updates' to the asset canister in the remainder of this document.

This section is relevant if your project contains an asset canister and describes how you can test handing over control of an asset canister to an SPS.
...
The general overview of deploying an asset canister during an SPS launch is as follows:
...
- Then, after launching the SPS, the SPS's function should be [registered](#sns-genericnervoussystemfunctions) to commit proposed changes.
- Last, using upgrade arguments, updated permissions can be set. This is only possible after the SPS canisters have been successfully deployed and [initialized](../launching/launch-summary-1proposal.mdx#7-automatically-sns-w-initializes-sns-canisters-according-to-settings-from-step-1) because before the launch the principal id of SPS governance is not known yet
    - The SPS governance canister is given `Commit` permissions. With `Commit`, SPS governance may apply a batch of proposed asset updates to the assets served by the asset canister.
    ...
    - To facilitate permission management via custom SPS proposals, it can make sense to grant SPS governance the `ManagePermissions` permission. With this permission, it is not necessary to upgrade the asset canister every time permissions are supposed to change.
...
Once the asset canister has been handed over to the SPS, only the governance canister should have `Commit` rights, and principals in the whitelist should have `Prepare` rights. The developer who configured and deployed the SPS should have their permissions removed prior to the SPS launch.
...
Once the asset canister has been deployed and permissions have been configured, the SPS decentralization swap can be started. You can learn more about launching an SPS [here](../launching/launch-summary-1proposal.mdx)
...
Therefore, to set the permissions such that SPS governance has `ManagePermissions` and `Commit` permissions and `principal1` has `Prepare` permissions, the upgrade argument would look like this:
  ...
  (variant {
    Upgrade = record {
      set_permissions = opt record {
        ...
        commit = vec { principal "<SPS governance>"; };
        manage_permissions = vec { principal "<SPS governance>"; };
      }
    }
  })
```

## SPS GenericNervousSystemFunctions

Generic proposals are a way for each SPS to define custom proposals. To commit changes to the asset canister, such generic proposals are used.

To use a generic proposal, first this new proposal type has to be "registered" in the SPS. To do so, one uses a `AddGenericNervousSystemFunction` proposal.

To submit a new `AddGenericNervousSystemFunction` SPS Proposal to support the `commit_proposed_batch` API, the target canister id should be the asset canister (that is updated in the update steps below) and the target function is `commit_proposed_batch`. The validate function should be `validate_commit_proposed_batch`.

The `ExecuteNervousSystemFunction` SPS proposal is used to execute the newly registered proposal. To submit an `ExecuteNervousSystemFunction` SPS Proposal with the output from `dfx deploy <frontend canister name> --network ic --by-proposal` see update steps below.

## Submitting an SPS proposal and upgrading an asset canister

Once an asset canister is under the control of the SPS, any changes must be done via an SPS proposal.

Before submitting an SPS proposal to update an asset canister, assure that the asset canister has been upgraded (by proposal) to use the Wasm bundled with [dfx 0.14.1+](https://github.com/dfinity/sdk/blob/release-0.14.1/src/distributed/assetstorage.wasm.gz).

- #### Step 1: To submit an SPS proposal, first have a principal with `Prepare` permission run:
...
An example of an SPS asset canister is canister `sqbzf-5aaaa-aaaam-aavya-cai`, which is an asset canister part of the [Dragginz Dapp SPS](https://dashboard.thebigfile.com/canister/sqbzf-5aaaa-aaaam-aavya-cai).
