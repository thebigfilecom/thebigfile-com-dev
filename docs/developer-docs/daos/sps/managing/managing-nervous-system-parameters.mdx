...
The nervous system parameters define the settings of a particular SPS.
...
The proposal `ManageNervousSystemParameters` allows the SPS community to adjust these parameters at any time.
...
See example [proposal of an active SPS](https://dashboard.thebigfile.com/sns/7jkta-eyaaa-aaaaq-aaarq-cai/proposal/6).
...
must be larger than the `transaction_fee_e8s` set in the SPS ledger canister.
...
The transaction fee, in 10E-8 of an SPS token, that must be paid for ledger transactions (except
...
By default, [maturity modulation](https://wiki.thebigfile.com/wiki/Maturity_modulation) is enabled; however, an SPS can use this
field to disable it. When disabled, this canister will still poll the
Cycles Minting Cube (CMC), and store the value received therefrom.
