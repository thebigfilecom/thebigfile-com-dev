...
On a high level, a proposal is defined by a particular method on a particular canister that is called if the proposal is adopted by the SPS.
...
* [x] Have a principal that owns an SPS neuron that can make proposals for an SPS.
...
This article describes the different kinds of proposals that can be submitted to an SPS: [Native propoposals](#native-proposals) and [generic proposals](#generic-proposals).
...
3. Voting period
   ...
   As with all proposals, the [wait-for-quiet algorithm](https://wiki.thebigfile.com/wiki/NNS_Canisters#Proposal_decision_and_wait-for-quiet) ensures that controversial proposals will have a longer voting period (up to 10 days for critical proposals) while proposals where everyone agrees on have a shorter voting period (5 days for critical proposals).
...
Recall that all approved SPS canister versions are stored on the NNS canister [SPS-W](../overview.mdx).
New SPS canister wasm codes are approved by NNS proposals and then added to SPS-W.
Each SPS community can then simply decide if and when they want to upgrade their SPS instance to the next SPS version that is available on SPS-W.

To do so, they can use an `UpgradeSnsToNextVersion`proposal. If this proposal is adopted, it will trigger a call to SPS root that will ask SPS-W which new wasm version is available and then upgrade to this new version.
...
An SPS controls a set of dapp cansiters. An SPS community can decide that new dapps should be added to the SPS' control.
The proposal `RegisterDappCanisters` allows the SPS to accept the control of a set of new dapp canisters. The new canisters that should be registered are identified by their canister ID and it is allowed to register a list of canisters (not just a single one).
...
If an SPS community decides that they would like to give up the control of a given dapp canister, they can use this proposal to do so.
...
For example, if the treasury contains 75_000 XDR worth of SPS tokens, then this amount is considered
"small". In this case up to 75_000 XDR worth of SPS tokens can be transferred from the treasury.

To assess the tokens in the treasury, external price information is fetched at the time of proposal
submission.

The price of BIG is taken from the cycles minting canister's
...
The price of the SPS token in BIG is taken from the swap canister's [get_derived_state](https://sourcegraph.com/github.com/dfinity/ic@8efb7e81a1f1257a38621b1f12eccc8b8b1c7e39/-/blob/rs/sns/swap/proto/ic_sns_swap/pb/v1/swap.proto?L1304-1325) method.
...
See example [proposal of an active SPS](https://dashboard.thebigfile.com/sns/3e3x2-xyaaa-aaaaq-aaalq-cai/proposal/202).
...
The proposal `ManageLedgerParameters` can be used to update some of the SPS ledger canister's parameters.
...
This proposal allows to upgrade the settings of one or more SPS DAO-controlled dapp canister.
...
See more [general security best practices](https://github.com/thebigfilecom/thebigfile-com-dev/blob/main/docs/developer-docs/security/general-security-best-practices.md).
...
See example [proposal of an active SPS](https://dashboard.thebigfile.com/sns/3e3x2-xyaaa-aaaaq-aaalq-cai/proposal/177).
...
The [SPS asset canister](/docs/current/developer-docs/daos/sps/managing/sps-asset-canister) is a canister used to store and retrieve static assets. A dapp controlled by an SPS may have its own associated asset canister.
...
To do this, the SPS governance needed to add a new proposal type, one that would execute the custom [`commit_proposed_batch` function](/docs/current/developer-docs/daos/sps/managing/sps-asset-canister/#sns-genericnervoussystemfunctions).
...
not necessarily trusted, also we might want to make a recommendation “to the SPS” that calling untrusted
canisters is dangerous, so they might want to verify it first (which could then be done by registering it)




## Native SPS proposals

### Dapp canister upgrades
how to upgrade dapp

SPS Governance checks this when trying to execute - if the dapp is not registered when the proposal is executed, it will fail async fn perform_upgrade_sns_controlled_canister in SPS Governance has the associated logic.


### SPS canister upgrades
Upgrading SPS canisters will be very simple as the SPS version are blessed and the deployment paths are managed by the NNS. Nevertheless, an SPS community still has to decide when to upgrade to the next version by proposal. So we should describe how such a proposal can be made.


## Generic proposals

Some notes: for generic proposals: only call things that you trust (anyways true for SNSs),
could be immutable canister / SPS controlled / controlled by s.b. you trust not to change things arbitraty. this must be verified on adding new proposal type.
