...
Each SPS can be individually configured with parameters that define, among other things,
how an SPS uses rewards to incentivize certain behavior for the governance participants
and the dapp users.

## Rewards Overview

The goal of this article is to explain the design of the SPS reward scheme.
...
decide what new features should be implemented in the dapp that is governed by the SPS.
...
SPS rewards. Based on the collected experience from developers and the community, this
can be enhanced in the future.

There are two categories of rewards:
  * **Voting rewards** to incentivize users to take part in SPS governance.

  * **User rewards** to incentivize dapp users to become early adopters and active users of the dapp that is governed by the SPS.

The reward scheme is based on the voting rewards used in the
[File Management System (FMS)](https://medium.com/dfinity/the-network-nervous-system-governing-the-internet-computer),
which however can be flexibly configured by each SPS.

## Recap on NNS voting rewards

The NNS is the DAO that governs the BigFile.
...
* Determination of the total reward pool:
  ...
  * The total pool of voting rewards for a given day is calculated as BIG supply (total supply of BIG tokens on that day) * R(t) / 365.25.
...
* Allocation of reward pool to neurons:
  ...
  * When a neuron is rewarded for voting, these rewards are recorded in an attribute of the neuron that is called maturity, which is not a tradable asset. If a user wants to generate income from maturity, they need to burn maturity to create new BIG via spawning a neuron which is a non-deterministic process described [here](https://wiki.thebigfile.com/wiki/Maturity_modulation).

## Design of SPS rewards
...
features of the NNS and describes how it is adapted and made configurable for the SPS.
Unless otherwise stated, the approach and formulae are the same as for the NNS.
As for the NNS, it is possible to change the SPS configuration by an SPS governance proposal.
...
* Start time for paying out rewards t<sub>start</sub>: timestamp greater than or equal to genesis time of the SPS. The start time is set to the current time once the reward calculation is switched on.
...
* The total pool of voting rewards for a given day is calculated as SPS supply (total supply of SPS tokens) * R(t) / 365.25.
* Voting rewards are minted, i.e. generating new supply once the according maturity is converted to the SPS token. In case that the SPS would like to stop a token supply increase after t<sub>start</sub>+t<sub>delta</sub> the SPS should set r<sub>min</sub>=0.
...
#### Allocation of reward pool
  ...
  * NNS has reward weights for different proposal types. For the first version of the SPS reward scheme this functionality is not yet available.

There is a flag which activates the calculation and distribution of voting rewards, as an SPS might choose to go through a ramp-up period without voting rewards, or with no voting rewards at all.

### Setting voting reward parameters

Voting reward parameters are defined in the SPS governance canister and can be changed by proposal. Details on the data structure **VotingRewardsParameters** used in the implementation are [here](https://github.com/dfinity/ic/blob/master/rs/sns/governance/proto/ic_sns_governance/pb/v1/governance.proto#L726).
...
* The purpose of user rewards is to foster early adoption and active usage of the SPS. Given that the meaning of usage and the according user rewards can vary greatly across individual SNSs, there is a very simple set-up at start.
* Some tokens (reserved for user rewards) can be held in an account that is owned by an SPS-controlled canister. This canister can then codify when the rewards are paid out and to whom.
