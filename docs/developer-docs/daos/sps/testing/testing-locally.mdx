...
To help developers, BigFile has created the [sns-testing repo](https://github.com/dfinity/sns-testing) which has scripts that help developers test the SPS process. Developers can run a local version of the BIG execution environment on their local machine, deploy their dapp locally and run through [the stages](../launching/launch-summary-1proposal.mdx) of decentralizing their dapp.
...
After having [chosen the initial SPS parameters in a .yaml file](../tokenomics/preparation.mdx) and before requesting an SPS launch in production, the SPS launch should be tested locally.
...
* If you have started using `sns-testing` before August 2023 and are using the old legacy flow of launching an SPS (please see [here](../launching/index.mdx) for more context), please continue your work with the legacy documentation on [this old README](https://github.com/dfinity/sns-testing/blob/v1-legacy/README.md). In particular,
...
### Testing stages of the SPS launch process

For simplicity, next the stages introduced in the [SPS launch stages documentation](../launching/launch-summary-1proposal.mdx) in the documentation [commands and actions to go through SPS launch](../launching/launch-steps-1proposal.mdx) are mapped to the relevant scripts in the `sns-testing` repo so you can learn what part of the launch is tested in which script.
...
### 2. Dapp developers choose the initial parameters of the SPS for a dapp.
...
### Testing the upgrading and managing of an SPS

Once a canister or dapp has been handed over to an SPS, it will be updated via proposals and voting. Developers can test this flow by [using commands from `sns-testing`](https://github.com/dfinity/sns-testing#sns-lifecycle).
...
<table border="1">
    ...
    <tr>
        <td>Upgrade a yet-to-be-decentralized canister by submitting an SPS proposal that can be voted on using the SPS developer neuron.</td>
...
You might also want to test integrations and basic SPS functionality.
For this purpose you can use the above instructions after the test SPS is
launched or use the [SPS testflight on mainnet](testing-on-mainnet.mdx).
