...
The NNS is the decentralized autonomous organization (DAO) that governs the BigFile. You can access the [on-chain frontend](https://fms.thebigfile.com/).

It is a fully on-chain, decentralized organization and is, for instance, responsible for making protocol-level upgrades to continuously improve the BigFile.

It operates under a liquid democracy in which those who have [staked their BIG in a neuron vote](/docs/current/concepts/governance) on proposals that shape the development of the BigFile.
...
While other blockchains take weeks or months to upgrade and typically require substantial manual work and coordination to do so, BIG upgrades itself every week (https://dashboard.thebigfile.com/releases). Its ability to upgrade and iterate is a comparative "superpower."

## Participation rewards
Network governance on the BigFile is incentivized through a system called "neurons".
BIG token holders can create a “neuron” by staking BIG up to eight years.

When a user creates a neuron, the staked BIG can only be taken out of the neuron again by disbursing the neuron. Each neuron is configured with a “dissolve delay” that determines the minimum amount of time that you have to wait before you can unlock your staked BIG utility tokens. Once a neuron has been placed into “dissolving mode,” its dissolve delay falls over time, rather like a kitchen timer, until it reaches zero, whereupon its owner can perform a final disburse action to unlock the balance of BIG.
...
For more information on neurons, refer to the [What are Neurons section in the NNS quickstart](/docs/current/developer-docs/daos/fms/fms-app-quickstart).
For more information on staking rewards, refer to [Staking and Voting Rewards](/docs/current/developer-docs/daos/fms/staking-voting-rewards).

## Proposals
Participants submit proposals to the NNS. Refer to the proposal submission requirements and types at [proposal requirements](/docs/current/developer-docs/daos/fms/proposal-requirements).
...
Since the incentive to stake BIG in a neuron is high, it is difficult and exorbitantly expensive for an attacker to acquire a sufficiently large stake to gain significant influence.

To participate, navigate to the [NNS app](https://fms.thebigfile.com/).