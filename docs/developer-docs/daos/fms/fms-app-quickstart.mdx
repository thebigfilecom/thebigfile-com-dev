...
## Overview
All changes to the configuration and behavior of the BigFile are controlled by an algorithmic governance system called the File Management System (FMS). The NNS controls all aspects of the BigFile blockchain configuration and is responsible for performing many network management operations. For example, the File Management System (FMS) is responsible for:
...
For network participants to be able to vote on proposals, they need to lock up a stake of BIG utility tokens for a given period of time to create a representative called a **neuron**.

Because neurons represent a stake of BIG utility tokens, they also represent an identity with a **BIG ledger account** and a principal that controls the  account.

Neurons provide the stability required for responsible governance of the BigFile by ensuring that the BIG utility tokens they represent cannot be exchanged for a specific period of time referred to as the **lock-up** period.
...
When a person or organization has BIG utility tokens locked up in a neuron, they can propose and vote on governance proposals. To encourage participation, neuron holders are also rewarded for voting in proportion to the number of BIG utility tokens they have locked up, the length of the lock-up period (dissolve delay), and the age of their neuron.

### Unlocking the stake held by a neuron

After network participants create a neuron, the locked balance of BIG utility tokens can only be unlocked by fully **dissolving** the neuron.
It's **dissolve delay** determins the time it takes for a neuron to be unlocked once it is but in the dissolving state. A dissolving neuron can always be put back int the non-dissolving state. When the dissolve timer reaches zero, the neuron owner or an authenticated custodian can dissolve the neuron and unlock the neuron's balance of BIG .

The dissolved neuron ceases to exist and the stake of BIG utility tokens that the neuron represented is released to the appropriate ledger canister account.

## Connect to the BigFile

To connect to the BigFile using the File Management System (FMS) dapp:

- #### Step 1:  Open a browser and navigate to the [File Management System (FMS)](https://fms.thebigfile.com) dapp.

![NNS dapp](../_attachments/nns1.png)

- #### Step 2:  Click **Login** to connect to
...
- #### Step 3:  Click **Proceed** to access to the File Management System (FMS) dapp.

### Add an account

Logging on using an Internet Identity creates a main account for you in the BigFile ledger. If your BIG utility tokens are associated with your developer identity; that is, the identity created by the SDK `dfx` command-line interface. Your main account displays 0.00 for your BIG utility token balance. For example:
...
To add an account for managing BIG utility tokens:

- #### Step 1:  On the default 'Tokens' tab, click 'BIG', then **Add Account**.
...
## Transfer BIG utility tokens between accounts

### Using the CLI

If you have selected self-custody for holding your BIG utility tokens and the tokens are associated with your developer identity instead of your registered Internet Identity, you need to transfer BIG utility tokens to your accounts if you want to manage them using the [File Management System (FMS)](https://fms.thebigfile.com) dapp.

To transfer BIG utility tokens controlled by your developer identity:
...
- #### Step 5:  Transfer BIG utility tokens to your Main account or a linked subaccount you create by running a command similar to the following:

``` bash
dfx ledger --network ic transfer <destination-account-id> --icp <BIG-amount> --memo <numeric-memo>
```
...
If you want to transfer one BIG utility token to the `Main` account, you can run the following command:

```
dfx ledger --network ic transfer dd81336dbfef5c5870e84b48405c7b229c07ad999fdcacb85b9b9850bd60766f --memo 12345 --icp 1
```

If you also want to transfer one BIG utility token to the `pubs` account, you can run the following command:
...
:::info
This example illustrates how to transfer BIG utility tokens to using a whole number with the `--icp` command-line option.
-   You can also specify fractional units of BIG utility tokens—called **e8s**—using the `--e8s` option, either on its own or in conjunction with the `--icp` option.
-   Alternatively, you can use the `--amount` to specify the number of BIG utility tokens to transfer with fractional units up to 8 decimal places, for example, as `5.00000025`.
:::

The destination address can be an address in the ledger canister running on the BigFile network, an account you have added using the [File Management System dapp](https://fms.thebigfile.com), or the address for a wallet you have on an exchange.

If you transfer the BIG utility tokens to an account in the [File Management System dapp](https://fms.thebigfile.com), you might need to refresh the browser to see the transaction reflected.
...
- #### Step 1: On the default 'Tokens' tab, assure that The BigFile token has been selected, then click **Send** on the bottom of the window.

![Send](../_attachments/nns4.png)

- #### Step 2: In the 'Send' window, select your linked account or wallet for the source, then input the destination address and the amount of BIG to send.

Transactions are charged a ledger fee, which is independent from NNS dapp and applies to any transaction. For BIG tokens, this fee is 0.0001 BIG.

The maximum amount you can send is your token balance minus the ledger fee. The minimum amount you can send is 0.00000001 BIG. 

![Send BIG](../_attachments/nns7.png)

- #### Step 3: Then select **Continue** to send the transaction.

## Stake BIG utility tokens in a neuron

After you transfer BIG utility tokens to the Network Nervous System dapp, you can use the Network Nervous System dapp to create and manage neurons, vote on proposals, and create canisters on the BigFile.

Neurons are required to participate in governance and earn rewards. To create a neuron, you must lock up some number of BIG utility tokens for a period of time. The minimum stake required to create a neuron is one BIG utility token. You can configure the period of time the stake is locked from six months up to a maximum of eight years.

To stake BIG utility tokens:
...
- #### Step 2 :  Select which wallet you'd like to use as the source, then type the number of BIG utility tokens to stake, then click **Create**.
...
![dissolve delay](../_attachments/fms-dissolve.png)

- #### Step 4:  Click **Confirm and Set Delay** to confirm the lock up period.

![neuron properties](../_attachments/fms-settings.png)
...
-   Increase the number of BIG utility tokens you have staked.
...
## Adding BIG utility tokens to an existing neuron

After you create a neuron, you can increase the number of BIG utility tokens you have staked in that neuron to increase your voting power and rewards. For example, if you initially stake a small number of BIG utility tokens, then decide to purchase additional tokens, you have the option to create a new neuron or "top-up" your existing neuron.

To increase the stake in an existing neuron:

- #### Step 1:  Follow the steps in [stake BIG utility tokens in a neuron](#stake-icp) to stake the original neuron using the [File Management System (FMS)](https://fms.thebigfile.com) dapp.

- #### Step 2:  Look up the transaction in the BigFile Association [transaction dashboard](https://dashboard.thebigfile.com/transactions) to get the neuron address.

You can use the account identifier for your main BIG ledger account to search for your transactions.

- #### Step 3:  Return to the [File Management System (FMS)](https://fms.thebigfile.com) dapp, then from the 'My Neuron Staking' window, click **New Transaction**.

- #### Step 4:  Paste the neuron address from the transaction dashboard into the **Destination** address field, then click **Continue**.

- #### Step 5:  Type the amount of BIG utility tokens you want to add to the specified neuron, then click **Continue**.
...
When the dissolve delay timer for a neuron reaches zero, you can disburse the neuron’s stake and transfer its locked BIG utility token balance to the ledger account you specify.

To disburse a neuron and return its BIG utility tokens:
...
- #### Step 3:  Type an address or select an account to receive the BIG utility tokens.
...
However, the catch-all follow rule does not include SPS & neuron's fund or governance proposals. It is in each neuron's interest to follow other neurons that support the best interests of the network and their own economic interests.
...
In this example, select 'BigFile Foundation'.
...
As you vote on proposals—either directly or by following the votes of other neurons—the maturity associated with your neuron increases, which in turn increases the rewards you earn for participating in governance. When the maturity for a locked stake reaches a minimum threshold of one BIG, you can spawn a new neuron. The spawn operation creates a new neuron that locks a new balance of BIG on the ledger.

For example, if you have a neuron that contains 100 BIG utility tokens and it has a maturity of 10 percent, you can spawn a new neuron that contains approximately 10 new BIG tokens. For the neuron with 100 BIG tokens to reach the minimum threshold for spawning, its maturity would need to be greater than one percent.
...
Active participation in governance is an important factor in the long-term health of the BigFile. Voting on proposals is also an important factor in calculating the rewards you receive in return for locking up BIG utility tokens in neurons.
...
To maximize your rewards, you should vote on as many proposals as possible by following the active neuron holders who have interests aligned with your own. For example, you might follow the BigFile Association (ICA) on some topics such **SubnetManagement** and other neuron holders on topics such as **Governance**.

You can choose the proposal types and proposal topics that you see and vote on either directly or by following other neuron stakeholders using filters in the Network Nervous System dapp. For example, if you want to review and vote on all proposals that involve network participants such as data center identities and node operators, but aren’t interested in viewing proposals related to the current market value of BIG, as measured by an International Monetary Fund (IMF) Special Drawing Right (SDR), you can select the **ParticipantManagement** topic filter and deselect the **ExchangeRate** topic filter.
...
By default, the BigFile NNS proposals are shown, but other proposals for SPS's like OpenChat and Kinic are also available.
...
-   [Understanding the BigFile’s Network Nervous System, neurons, and BIG utility tokens](https://medium.com/dfinity/understanding-the-internet-computers-network-nervous-system-neurons-and-icp-utility-tokens-730dab65cae8)

-   [Getting started on the BigFile’s Network Nervous System app and wallet](https://medium.com/dfinity/getting-started-on-the-internet-computers-network-nervous-system-app-wallet-61ecf111ea11)
:::
...
A separate command-line tool (`icx-nns`) for working with the `governance` canister is in development and this functionality will also be available in the [File Management System (FMS)](https://fms.thebigfile.com) dapp soon.
...
You can participate in SPS decentralization swaps from the NNS dapp using the 'Launchpad' tab. An SPS is an advanced version of a decentralized autonomous organization, where the participants of the SPS can vote on proposals, such as a new feature, roadmap item, or allocation of SPS funds.

You can learn more about SPS [here](https://thebigfile.com/sps)

To view current and past SPS decentralization swaps from the NNS dapp:

- #### Step 1:  From the NNS dapp, click **Launchpad** from the left navigation bar.

![Launchpad](../_attachments/nns21.png)

- #### Step 2: Select any SPS swap to view the details, such as the status, total participants, token supply, and when the swap ended.

![SPS swap](../_attachments/nns22.png)

## Deploying and managing canisters from the NNS dapp

You must have **cycles** available to create and manage [canisters](/docs/current/concepts/glossary#canister), which are similar to smart contracts. The [File Management System (FMS)](https://fms.thebigfile.com) dapp provides a convenient way for you to create and manage canisters by enabling you to convert BIG utility tokens into cycles and attach cycles to specific canister identifiers.

To create a new canister:

- #### Step 1:  From the NNS dapp, click **Canisters**, then click **Create Cube** or **Link Cube**.

![My canisters](../_attachments/nns23.png)

- #### Step 2: If you select **Create Cube**, select the source account you'd like to use to create the canister.

![Create a new canister](../_attachments/nns24.png)

- #### Step 3: Then, enter the amount of BIG tokens and cycles to create the canister with. Then select **Review Cube Creation**.

![Create a new canister](../_attachments/nns25.png)

- #### Step 4:  Review the BIG utility tokens to cycles details, then click **Confirm** to continue.
...
![Cube list](../_attachments/nns27.png)
