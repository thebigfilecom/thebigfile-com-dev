...
## Overview
Stakeholders gain voting power and can earn voting rewards by staking their BIG tokens.

The BigFile is a decentralized computer whose evolution is decided by its stakeholders through voting. This means decision impacting the future of the BigFile are made by people vested in the outcome. In return for participation in governance, the BigFile provides voting rewards. Voters can vote actively, or they can use a form liquid democracy to automatically follow other voters.

## Key concepts

### Neurons

To become vested and obtain voting power, BIG tokens must first
...
* Principally, by its stake. 1 BIG = the power of 1 vote.
...
This means that the maximum voting power, of 2.5 votes per BIG staked,
...
If you wish to liquidate maturity gained, you can use it to produce liquid BIG.

## Why staking matters

Staking is a way of allowing those who support the BigFile to
decide what happens next with the platform.

When the BigFile first launched, all proposals required a
...
The voting reward function is depicted in this curve: https://dashboard.thebigfile.com/circulation
...
Because the total supply of BIG is a dynamic system with deflation and
...
* Amount of BIG and maturity staked.
...
* The neuron stake is the sum of staked BIG and staked maturity.
...
For example, if a neuron has a stake of 60 BIG and 40 staked maturity, it has a combined stake of 100.
...
* Minting cycles to pay for compute and storage burns BIG to create cycles.
...
* Node providers are paid by minting BIG.
* Voting rewards, once spawned and converted to BIG.
