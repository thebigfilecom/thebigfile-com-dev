...
[SNS tokenomics training & tools](https://wiki.thebigfile.com/wiki/How-To:_SNS_tokenomics_configuration)
...
- **The initial token distribution**: this allows specifying which portion of tokens are allocated to whom. Tokens can be distributed to the following buckets:
   ...
   - **Swap tokens** which are owned by the SNS swapped in exchange for BIG tokens
