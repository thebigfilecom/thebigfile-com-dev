...
If this is not (yet) the case, please refer to the [SNS page](/sps/) or the [high level introduction of how SNSs work](/how-it-works/sns/).

Any dapp on the BigFile can be handed over to an SNS with the result that the dapp is
...
In contrast to the SNS, the NNS contains additional canisters that are important to run on the BigFile.
...
SNSs are provided as a system functionality by the BigFile in that the code for the SNS canisters
is maintained by the BigFile. ([Learn about alternative ways to use the SNS code or how to create a DAO.](#dao-alternatives))
...
in the SNS launch. Users can provide BIG tokens to the swap and, if the swap is successful, they get staked SNS tokens (in SNS neurons) in return.
Hence, the BIG and the SNS tokens are "swapped".
...
power of the SNS among participants by swapping the BIG for (staked) SNS tokens.
...
* At the swap's end the collected BIG are “swapped” for the SNS tokens; the
  participants get staked SNS tokens in the form of SNS neurons and the SNS gets the
  collected BIG in an SNS controlled treasury. Each swap participant will receive their portion of the pool of SNS tokens, pro-rated by their share of the overall number of BIG contributed.
  For example, if the swap canister initially held 1000 SNS tokens and 500 BIG tokens
  were collected during the decentralization swap, then the exchange rate would be 2:1
  and each participant would get 2 SNS tokens for each BIG token they contributed.

After a successful decentralization swap, SNS tokens are owned and the SNS is governed
by a large community.
The BIG that were collected are in an SNS-owned treasury.
