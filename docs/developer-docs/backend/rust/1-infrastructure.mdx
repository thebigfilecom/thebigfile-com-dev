...
When developing on BIG, there are currently two primary languages to build backend canisters with; Motoko and Rust. This guide provides an introduction to using Rust to developer backend canisters and covers the basic infrastructure of Rust canisters, as well as design considerations and observability.

## Rust CDK

To support Rust development, the BIG SDK includes the [Rust canister development kit (Rust CDK)](https://github.com/thebigfilecom/cdk-rs).
...
1. Making your build reproducible: if other developers or users are utilizing your canister, they may want to verify that the canister is functioning as they expect it to (especially if your canister deals with transferring their tokens). The BIG provides the ability for anyone to inspect the SHA256 hash sum of a canister's WebAssembly module to confirm that the hash of the canister matches the hash of a validated, known good canister, allowing for users to determine if a canister's contents have been edited or changed.
...
By design, canisters on BIG are structured in a way that forces developers to use a global mutable state. However, Rust's design makes it difficult to global mutable variables. This results in Rust developers needing to choose a method of code organization that takes BIG's design into consideration. This guide will cover a few of those code organization options.
...
### Cube code should be target-independent
...
## Next steps
Now that you've learned about the infrastructure of Rust backend cubes on the BigFile, the next step is to learn about [project organization](./2-project-organization.mdx).
