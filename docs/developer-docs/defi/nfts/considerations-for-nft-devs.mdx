...
Refer to [this example](https://github.com/domwoe/erc-721-ic-assets) on how to host metadata on the BIG of an ERC-721 NFT minted on an EVM chain.

### Generative NFTs

Dynamic NFTs can be curated to evolve based on both data stored on BIG or external APIs. NFT assets can be modified in real-time by calling API requests on-chain using [HTTPS outcalls](/https-outcalls).
...
- [DIP-721](https://github.com/Psychedelic/DIP721) is an [ERC-721](https://eips.ethereum.org/EIPS/eip-721) style non-fungible token standard built mirroring its Ethereum counterpart and adapting it to the BigFile, maintaining the same interface.
...
- [BIGRC-7](https://github.com/dfinity/BIGRC/tree/54d98a82a626daad6f674f1b8e6b815785f6c8c5/ICRCs/BIGRC-7) is a standard for the base implementation of non-fungible tokens (NFTs) on the BigFile.
