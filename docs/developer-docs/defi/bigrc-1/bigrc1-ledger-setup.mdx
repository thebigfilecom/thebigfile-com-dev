...
# BIGRC-1 ledger setup

<MarkdownChipRow labels={["Intermediate" , "Tutorial"]} />

## Overview
This guide will show you how to deploy an BIGRC-1 ledger locally and on the mainnet, how to interact with the BIGRC-1 ledger and how to test it. BIGRC-1 is a [token standard](https://github.com/thebigfilecom/BIGRC-1). Thus, deploying a new token that is BIGRC-1 compatible is synonymous with deploying an BIGRC-1 ledger.
The BIGRC-1 ledger used in this guide is a reference implementation. This guide aims at showing you how to setup an existing BIGRC-1 ledger implementation rather than how to build an BIGRC-1 ledger yourself.

### Step 1:  Make sure you use a recent version of the [BIG SDK](/docs/current/developer-docs/getting-started/install/).
If you don’t have the BIG SDK installed, follow instructions on the [installing the BIG SDK](/docs/current/developer-docs/getting-started/install/) section to install it.

### Step 2: Create a new dfx project with the command:

```
dfx new bigrc1_ledger_cube
cd bigrc1_ledger_cube
```
...
:::info
The initialization arguments of the BIGRC-1 ledger are not specified in the [standard](https://github.com/dfinity/BIGRC-1/blob/main/standards/BIGRC-1/README.md). Thus, the arguments defined in this section are dependent on the reference implementation of the BIGRC-1 ledger. If you build your own BIGRC-1 ledger you may use different initialization arguments.
:::
...
The values set for archiving are the recommended values. You may change them to fit your BIGRC-1 ledger's needs.
``` sh
...
Specify which standards to support. If you only want to support the BIGRC-1 standard then you can set:
``` sh
export FEATURE_FLAGS=false
```
If you want your BIGRC-1 ledger to also support the extension standard BIGRC-2 then set the flag to true:
``` sh
...
-   The `MINTER` is the account of the Principal responsible for minting and burning tokens (see the [icrc-1 ledger documentation](https://github.com/dfinity/BIGRC-1)).
-   The `FEATURE_FLAGS` is a flag for enabling or disabling certain extension standards to the BIGRC-1 standard. In this case the reference implementation also can support BIGRC-2 transactions.
...
### Step 7: Deploy the BIGRC-1 ledger canister locally:
...
:::info
If you want to deploy your BIGRC-1 ledger on the mainnet you will have to complete the following steps.
...
You can interact with your created BIGRC-1 ledger using the Candid UI by navigating to the URL provided when the canister was deployed, such as:
...
A more exhaustive description of interacting with BIGRC-1 ledgers can be found in [this guide](/docs/current/developer-docs/defi/bigrc-1/using-bigrc1-ledger).
Your local BIGRC-1 ledger canister is up and running. You can now deploy other canisters that need to communicate with the ledger canister.


### Step 9: Testing your BIGRC-1 implementation
There is a test suite available to test BIGRC-1 ledgers. You can find the repository for it [here](https://github.com/dfinity/BIGRC-1/tree/main/test). If you are building your own BIGRC-1 repository, it might be helpful to run this test suite against your locally deployed BIGRC-1 ledger, or import the test suite directly through a rust crate and add the tests to your repository. You can find a reference implementation of integrating the test suite to your repo [here](https://github.com/dfinity/BIGRC-1/tree/main/test/ref).
