...
```
If you want your BIGRC-1 ledger to also support the extension standard ICRC-2 then set the flag to true:
``` sh
...
-   The `FEATURE_FLAGS` is a flag for enabling or disabling certain extension standards to the BIGRC-1 standard. In this case the reference implementation also can support ICRC-2 transactions.
