...
## Overview
This guide will show you how to deploy an BIG index canister locally, how to connect it to your local BIG ledger, and how to interact with the BIG index canister.

If you are working in a local development environment, i.e with a local replica instead of the public BigFile, you can't access the BIG ledger nor the BIG index canister. If your application is using the BIG index canister and you want to test it, you can setup the BIG index and BIG ledger locally. Neither of the two canisters will have any information about the state of the BIG ledger on the mainnet. You will have to create your own transactions on the BIG ledger so that the BIG index can serve them through its endpoints.
...
It is advised you use the same project folder that you created during the local ledger setup. Alternatively, you can create a new one for the BIG index canister using the following command:
...
The URL for the BIG index Wasm module is `curl -o index.wasm.gz "https://download.dfinity.systems/ic/$IC_VERSION/canisters/ic-icp-index-canister.wasm.gz"`, so with the above revision it would be `https://download.dfinity.systems/ic/d87954601e4b22972899e9957e800406a0a6b929/canisters/ic-icp-index-canister.wasm.gz`.
...
Open the `dfx.json` file in your project's directory. Add the `icp_index_canister` canister data and insert the canister data for your BIG ledger. If you followed the guide on local ledger setup and you used the same project folder for both the BIG ledger and BIG index, your `dfx.json` file should look like this:
...
### Step 6: Deploy the BIG index canister:
Here it is assumed that the canister ID of your local BIG ledger is `ryjl3-tyaaa-aaaaa-aaaba-cai`, otherwise replace it with your BIG ledger canister ID.
...
The BIG index canister will start synching right away and will automatically try to fetch new blocks from the BIG ledger every few seconds.

### Step 7: Check the status and BIG ledger id on the BIG index canister.
...
You can use the BIG index canister to fetch blocks like so.
