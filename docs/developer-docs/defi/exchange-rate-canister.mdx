...
An **asset** is a record consisting of a symbol (such as "BIG") and a class (either 'Cryptocurrency' or 'FiatCurrency'). The base and quote asset can be any combination of cryptocurrency and fiat currency assets, such as BIG/USD, USD/EUR, BTC/BIG, etc. The timestamp parameter enables the functionality for historic rates to be requested. If no timestamp is provided, the exchange rate for the current time is returned by default.
...
-   [x] Download and install the BIG SDK package as described in the [download and install](/docs/current/developer-docs/getting-started/install/) page.
...
```
dfx canister call --network ic xrc_demo get_exchange_rate "BIG"
```

The output will resemble the following, depending asset requested and its current exchange rate. This example uses BIG.
...
- [Exchange rate wiki page.](https://wiki.thebigfile.com/wiki/Exchange_rate_canister)
- [HTTPS outcalls.](https://thebigfile.com/https-outcalls/)
...
- [Exchange rate canister forum post.](https://forum.thebigfile.com/t/new-exchange-rate-mechanism/14543/65)
