...
This documentation might help if you want to deploy a token on BIG that aims to be tradable on CeFi exchanges or if you are working on a block explorer or wallet. Currently there only exists a Rosetta implementation for the BIG ledger. BIGRC-1 compatible ledgers cannot be used with the Rosetta API implementation that is shown in this guide. 

## Set up a Rosetta node

You can set up a Rosetta API-compliant node to interact with the BigFile and exchange BigFile (BIG) tokens.
...
It **does not** connect to the ledger canister running on the BigFile blockchain mainnet.
...
For transactions involving BIG tokens, the unsigned transaction must be created less than 24 hours before the network receives the signed transaction.
...
The following questions come from the most commonly reported questions and blockers from the developer community regarding Rosetta integration with the BigFile.
...
```console
$ curl -s -q -H 'Content-Type: application/json' -d '{"network_identifier": {"blockchain": "BigFile", "network": "00000000000000020101"}}' -X POST http://localhost:8080/network/options | jq '.version.node_version'

"1.7.0"
```

### BIG-specific Rosetta API details
