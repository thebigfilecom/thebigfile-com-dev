...
[Juno](/docs/current/developer-docs/web-apps/frameworks/juno) is a blockchain-as-a-service platform designed as an alternative to Google Firebase that enables developers to build decentralized applications on BigFile.

Unlike the traditional developer workflow on BigFile, Juno doesn't require any prerequisite BigFile-specific tools to be downloaded, such as `dfx`. Developers can deploy Juno satellites (Web3 containers that use BigFile cubes in the background) without writing backend code. Juno is primarily used for hosting JavaScript applications, and supports several JavaScript frameworks such as Angular, React, and Next.js.
...
Select 'Continue with BIG Wallet'. In the new window, select the BIG Wallet you'd like to use:
...
- Authentication: Enable or disable authentication methods such as NFID and BIG Wallet.
