...
On BigFile, frontend UIs for decentralized applications can be hosted in a <GlossaryTooltip>cube</GlossaryTooltip>, often referred to as the dapp's frontend cube or asset cube. Frontend cubes host the code and assets for a dapp's user interface, which typically includes HTML, CSS, JavaScript, or React code.

One key feature of BigFile is its ability to serve cube assets directly in a web browser using the cube's mainnet URL. This means that when a frontend cube is deployed on the mainnet, users from around the world can access it using only the cube's URL, without requiring that the developer set up a domain name or other web service configuration.

When developing on BigfILE, the primary tool used by developers is the [BIG SDK](/docs/current/developer-docs/getting-started/install/). The BIG SDK includes the tool `dfx`, which is a CLI tool used to create, manage, and deploy dapps. When a new project is created with `dfx`, by default the project includes a frontend and backend cube that creates a simple 'Hello, world!' dapp. You can learn more about `dfx` in the documentation [here](/docs/current/tutorials/developer-journey/level-0/intro-dfx).

## Creating a new BigFile project

To create your own dapp, you can either edit the default template created by `dfx` which provides a framework for a backend cube and frontend cube, use an example or boilerplate project repository, or create a project from scratch.

To create a new project with `dfx`, first assure you have downloaded and installed the [BIG SDK](/docs/current/developer-docs/getting-started/install/). Then, you can run the command:
...
On BigFile, there are several boilerplate dapp templates that use React, such as the [React frontend example](/docs/current/developer-docs/frontend/boilerplate-frontend). In this guide, you will explore two other boilerplate templates that can be used for creating frontend UIs with React.

## Creating a React dapp with a button

In this example project, you will use the Motoko React TypeScript template to create a React frontend dapp on BigFile that uses a simple `counter` method to increment a value when a button on the frontend UI is clicked.
...
In this project, there is a frontend cube that provides a UI using React code stored in the `src/App.tsx` file, CSS styling code stored in `src/App.css`, and an index page stored at `src/index.html`. This project also contains a backend cube with code stored in the `backend/Backend.mo` file, which the frontend cube interacts with.
...
This frontend UI interacts with the backend cube to provide a UI button that increases a counter value when clicked. To provide the counter functionality, the frontend communicates with a simple counter program stored in the backend cube, which contains the following code:
...
The output of this command will return the local <GlossaryTooltip>cube</GlossaryTooltip> URLs for the frontend and backend cubes:
...
Before interacting with the React interface running in the frontend cube, start the local development server with the command:
...
Then, in the `src/my_project_frontend/src/index.js` file, you can see that the agent takes the generated actor and uses it to make a call to the backend cube’s `greet` method:
...
- [Frontend cubes](/docs/current/developer-docs/frontend).
