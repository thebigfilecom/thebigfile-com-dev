...
When developing on BigFile, there are two primary tools used: `dfx`, a CLI tool used
...
- [Motoko](/docs/current/motoko/main/getting-started/motoko-introduction): BIG's native programming language.
...
`dfx` is part of the [BIG SDK](https://github.com/dfinity/sdk).
...
<AdornedTabs>
  ...
  <TabItem value="windows" label="Install on Windows">
    There is no native support for <code>dfx</code> on Windows. However, you can develop BIG canisters on Windows using a Docker container or by installing the Windows Subsystem for Linux (WSL).
    ...
    Open the Docker Desktop application to start the Docker Daemon. Then, open a command line window and pull the DFINITY BIG developer environment Docker image:
...
[BIG SDK repo](https://github.com/dfinity/sdk). The README contains the
