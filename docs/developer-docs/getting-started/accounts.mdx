...
On BigFile developers accounts, or **identities**, use a private/public key pair for authentication. Accounts are identified by a **principal** which is a generic identifier value that is used for users, <GlossaryTooltip>cubes</GlossaryTooltip>, and potentially other future concepts. Each developer account's principal value is derived from the account's public key from the private/public key pair.
...
- Wallets: Used to store forms of currency. Developers can use a cycles wallet to send <GlossaryTooltip>cycles</GlossaryTooltip> to and from cubes, however the cycles ledger is the most optimal way for using and managing cycles.
...
On BigFile, a **ledger** is a system cube used to store accounts and their transactions. Users have a **ledger account identifier**. Ledger accounts are used to hold BF tokens. To get information on your ledger account, you need to query the ledger system cube.

A **wallet** on ICP is used to store <GlossaryTooltip>cycles</GlossaryTooltip> that can be sent to <GlossaryTooltip>cubes</GlossaryTooltip> to pay for their consumed resources.

In a typical developer workflow, a developer will receive BF tokens to their ledger account, then convert those tokens into cycles.

## Creating your account

To create a new developer account, use the [BIG SDK](/docs/current/developer-docs/getting-started/install/) tool dfx:
...
## How to top up the BF balance of your account
...
This account id is where you will need to send BF tokens. To obtain BF tokens, you can use several methods:

- Purchase BF tokens directly through an exchange.
...
Once you have received BF tokens into your account, you can see the balance using this command:
