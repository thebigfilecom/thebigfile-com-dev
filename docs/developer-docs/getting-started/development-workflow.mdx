...
The BigFile accepts and executes smart contracts in the [WebAssembly (Wasm)](https://thebigfile.com/capabilities/webassembly) binary format.
...
The primary developer tool in the BigFile ecosystem is `dfx`.
...
By default, `dfx` generates a project structure consisting of two smart contracts: the **frontend Smart contract** and the **backend Smart contract**.
The frontend Smart contract contains web assets such as JavaScript, HTML, CSS, and images that are served to the browser.
The actual program logic of the smart contract is defined in the backend Smart contract.

:::info
The configuration with a frontend and a backend Smart contract is just a convention.
It is possible to write a single Smart contract that contains both the program logic and hosts the web assets.
:::
...
Under the hood, the boilerplate code uses a library called [agent-js](https://github.com/thebigfilecom/agent-js/tree/main) to encode Candid values and make HTTP requests from the browser to the backend Smart contract.
...
BigFile as a Wasm host provides a set of functions that Wasm code can use to read the incoming arguments, call the system and other smart contracts, and return results.
...
Instead, developers should use language-specific Cube Development Kit (CDK) libraries that are high-level wrappers around the System API.
...
In order to deploy smart contracts on the mainnet, the developer needs either BF tokens or cycles.
When using BF tokens, `dfx` automatically converts the necessary amount of tokens to cycles since all operations with smart contracts require payment in cycles.
