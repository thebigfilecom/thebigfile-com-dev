...
When developing <GlossaryTooltip>cubes</GlossaryTooltip> and projects, it is recommended to first deploy your project to the local development environment. This is so you can fabricate <GlossaryTooltip>cycles</GlossaryTooltip> in your local development environment, which you cannot do on the mainnet. Cycles are charged for the resources used by your cube. You can learn more about cycles [here](/docs/current/developer-docs/gas-cost).

Deploying to your local development environment is similar to deploying to a testnet. BigFile does not have a testnet available for developers to deploy to. Instead, the BIG SDK runs a local version of the BigFile node software known as the **replica** and provides a single-node environment on your local machine for you to deploy cubes to.

## Prerequisites

Before you create your first cube, verify the following:
...
Before you can build your first project, you need to connect to the local <GlossaryTooltip>replica</GlossaryTooltip>. As a best practice, this step requires you to have **two terminal shells** open, so that you can start and see cube execution operations in one terminal and manage your project in another.
...
- #### Step 4:  Leave the terminal window that displays cube execution operations open and switch your focus to the first terminal window where you created your new project.

    You should perform the remaining steps in the terminal that doesn’t display cube execution operations.

## Register, build, and deploy the application

After you connect to the local cube execution environment you can register, build, and deploy your dapp locally.

To deploy your first cube locally:

- #### Step 1:  Check that you are still in the root directory for your project, if needed.

It is recommended to follow the [creating your first canister](/docs/current/developer-docs/getting-started/default-template) guide to have a simple 'Hello, world!' cube ready to deploy.
...
- #### Step 3:  Register, build, and deploy your first dapp by running the following command:
    ...
    If you created a project with a different name, however, your cube names will match your project name instead of `hello_backend` and `hello_frontend`.

## Call the cube from the CLI

Call the `hello_backend` cube and the predefined `greet` function by running the following command:
    ...
    Remember, however, that if you created a project with a different name, the cube name will match your project name and you’ll need to modify the command line to match the name you used instead of `hello_backend`. If you were to choose `test` as the projects name, your backend cube would be called `test_backend` and the frontend cube `test_frontend`.
...
## Call the cube from browser

It is possible to access the frontend cube directly from the browser. You will need to navigate to the URL that was printed in the output of the `dfx deploy` command (your URL might differ):
...
This will render a webpage with a form that accepts a string and sends it to the backend cube.
...
The JavaScript code running in browser uses the [`agent-js`](/docs/current/developer-docs/agents/javascript-intro) library to interact with the backend cube. The library creates a wrapper JavaScript object based on the interface of the cube. That wrapper object provides convenience methods for calling the cube functions as if they were JavaScript functions:

```
const greeting = await hello_backend.greet(name);
```

## Stop the local cube execution environment

After testing the application in the browser, you can stop the local cube execution environment so that it doesn’t continue running in the background.
...
- #### Step 2:  In the terminal that displays cube execution operations, press Control-C to interrupt the local network process.
