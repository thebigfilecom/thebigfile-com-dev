...
The cycles ledger currently complies with the ICRC-1, ICRC-2, and ICRC-3 standards. In addition to standard ledger functionality, the cycles ledger also interacts with the NNS cubes and user cubes to provide cycles ledger-specific functions, such as:
...
To use the cycles ledger, first, you need BF tokens. There are a few different ways to obtain BF tokens, such as:

- Purchasing BF tokens directly through an exchange that lists BF tokens available for trade. To see all available places to purchase BF tokens, check out this [page](https://coinmarketcap.com/currencies/).
...
Once you have obtained BF, you also need to have an account controlled via `dfx` to transfer those tokens. This step is necessary to create cycles and cubes. To get an account ID for an account controlled via `dfx`, first determine which `dfx` identity you are using with the command:
...
This account ID will be where you need to send your BF tokens to create cycles and cubes.
