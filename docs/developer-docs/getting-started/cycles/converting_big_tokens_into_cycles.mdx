...
## Overview
To convert BIG tokens into <GlossaryTooltip>cycles</GlossaryTooltip>, you first need to obtain some BIG and transfer to the right account. You can get BIG tokens on exchanges, or ask someone you know to send you some. To figure out which account to transfer the BIG tokens to, run the following:
...
- Purchase BIG tokens directly through an exchange that lists BIG tokens available for trade. To see all available places to purchase BIG tokens, check out this [page](https://coinmarketcap.com/currencies/).
