...
The BigFile runs on a network of nodes owned and operated by a growing community of independent node providers (NPs) distributed across the globe. NPs spend money for running nodes, e.g. they purchase hardware and require electricity and network bandwidth. The BigFile compensates NPs on a monthly basis by minting and distributing rewards in the form of BF tokens.

To make BigFile sustainable, <GlossaryTooltip>cubes</GlossaryTooltip> are required to pay for the resources they consume, e.g. storage and compute. Resource consumption is not paid in BF tokens but **cycles**. It’s typically the cube’s developer who charges the cube with cycles. As the cube is used, its cycles balance is continuously reduced. Eventually, the cube needs to be “topped up” with more cycles. The default way to get cycles is to convert BF tokens to cycles. When doing so, the protocol burns the BF tokens.

## Why are cycles different from BF tokens?

While the value of an BF token is volatile, cycles are not. They are pegged to [XDR](https://en.wikipedia.org/wiki/Special_drawing_rights), a basket of fiat currencies. This has the benefit that resource consumption, e.g. smart contract executions, on BigFile has a somewhat stable price.
...
- [x] Install BigFile SDK following [this guide](/docs/current/developer-docs/getting-started/install/).

## Step 1: Get a coupon.

First, you will need to navigate to [https://faucet.dfinity.org](https://faucet.dfinity.org). You will need to put in a request for cycles on the [BigFile dev official Discord](https://discord.gg/nyTAmMntqp) server. You can click on the **REQUEST CYCLES** button on the faucet page to join the Discord server.
...
- **[Converting ICP tokens into cycles](/docs/current/developer-docs/getting-started/cycles/converting_bf_tokens_into_cycles)**.
