...
As discussed in [tokens and cycles](/docs/current/concepts/tokens-cycles), BIG tokens can be converted into **cycles** to power <GlossaryTooltip>cube</GlossaryTooltip> operations. Cycles reflect the operational cost of communication, computation, and storage that dapps consume.

Unlike BIG tokens, cycles are only associated with cubes and not with user or developer <GlossaryTooltip>principals</GlossaryTooltip>. Because only cubes require cycles to perform operations and pay for the resources they use, users and developers can manage the distribution and ownership of cycles through a special type of canister called a **cycles wallet**.
...
The cycles wallet holds the cycles required to perform operations such as creating new cubes. These operations are executed using the cube principal of the cycles wallet instead of your user principal.

For the purposes of using the local cube execution environment, the SDK automatically creates a default cycles wallet for you in every project and most of the operations performed using the cycles wallet happen behind the scenes. For example, the cycles wallet acts on your behalf to register cube principals and deploy cubes in the local cube execution environment.

In a production environment, however, you need to explicitly register and transfer cycles to new cubes, specify the principals that can act as custodians, and manage the principals with ownership rights. You can perform some of these tasks using the default cycles wallet dapp running in a web browser. Depending on the specific action you want to take, you can also perform these cycle and cube management tasks by running `dfx wallet` commands in a terminal or by calling methods in the default cycles wallet cube directly. You can learn more about [how to use `dfx wallet`](/docs/current/developer-docs/developer-tools/cli-tools/cli-reference/dfx-wallet).

You should keep in mind, however, that calls to the cycles wallet cube are executed using the cycles wallet principal associated with the currently selected user identity. Depending on your currently selected identity and whether the principal associated with that identity has been added as a controller or a custodian for a wallet, you might see different results or be denied access to a specific method.
...
:::info
The controller role this document talks about is NOT the same controller role usually meant when talking about the BigFile. Typically, the controller refers to a <GlossaryTooltip>principal</GlossaryTooltip> that controls a cube. Here, a controller is a **wallet-internal** role that happens to have the same name. For a more detailed differentiation, see [this forum post](https://forum.thebigfile.com/t/why-is-my-cycles-wallet-canister-slowly-losing-cycles/13190/11).
:::
...
-   Send cycles to other cubes.

-   Accept receipt of cycles from other cubes.

-   Rename the cycles wallet.

-   Create cubes and additional cycles wallets.

:::caution
A cube can have a maximum of 10 controllers. Learn more [here](/docs/current/references/ic-interface-spec#ic-create_canister).
:::
...
- [x] Install the [BIG SDK](/docs/current/developer-docs/getting-started/install/).

### Creating a local cycles wallet

If you are doing local development, your local cycles wallet is created when you register a new cube principal using `dfx canister create` or when you register, build, and deploy a cube with `dfx deploy`.

### Creating a cycles wallet on the mainnet

If you are deploying on the BigFile mainnet, you typically create your cycles wallet by converting BIG tokens to cycles, transferring the cycles to a new cube principal, and updating the cube with the default cycles wallet WebAssembly module (Wasm).

There are dapps that can help you convert BIG to cycles and create a new cycles wallet, e.g., [NNS dapp](/docs/current/developer-docs/daos/fms/fms-app-quickstart#_deploy_a_canister_with_cycles).

- #### Step 1:  Create a new canister with cycles by transferring BIG tokens from your ledger account. You can do so by running a command similar to the following:
```
dfx ledger --network ic create-canister <principal-identifier> --amount <big-tokens>
```
This command converts the number of BIG tokens you specify for the `--amount` argument into cycles, and associates the cycles with a new cube identifier controlled by the principal you specify.

For example, the following command converts .25 BIG tokens into cycles and specifies the principal identifier for the default identity as the controller of the new cube:
```
...
Cube created with id: "gastn-uqaaa-aaaae-aaafq-cai"
```
- #### Step 2:  Install the cycles wallet code in the newly-created cube ID by running a command similar to the following:
```
...
```
Creating a wallet canister on BIG.
...
In the local cube execution environment or with a cycles wallet on the BigFile, you can use the following command to check the current cycles balance:
...
After you have used the `dfx identity deploy-wallet` command to create a cycles wallet cube tied to an identity, you can use `dfx wallet` commands to modify your cycles wallet settings, send cycles to other cycles wallets, and add or remove controllers and custodians.
...
Use the `dfx wallet` command with subcommands and flags to manage the cycles wallets of your identities and to send cycles to the wallets of other account cycles wallet cubes.
...
- [`send`](/docs/current/developer-docs/developer-tools/cli-tools/cli-reference/dfx-wallet#send): sends a specified amount of cycles from the selected identity's cycles wallet to another cycles wallet using the destination wallet cube ID.
...
The default cycles wallet cube includes additional methods that are not exposed as `dfx wallet` commands. The additional methods support more advanced cycles management tasks such as creating new cubes and managing events.

### Create a new cycles wallet

Use the `wallet_create_wallet` method to create a new cycles wallet cube with an initial cycle balance and, optionally, with a specific principal as its controller. If you don’t specify a controlling principal, the cycles wallet you use to create the new wallet will be the new wallet’s controller.
...
```
### Register a new cube principal

Use the `wallet_create_canister` method to register a new cube principal on the BigFile. This method creates a new "empty" cube placeholder with an initial cycle balance and, optionally, with a specific principal as its controller. After you have registered the cube principal, you can install code for your cube as a separate step.
...
```
The command returns the principal for the new cube you created:
```
(record { 1_313_628_723 = principal "dxqg5-iyaaa-aaaab-qaawa-cai" })
```
### Receive cycles from a cube
...
```
If the cycles wallet (`gastn-uqaaa-aaaae-aaafq-cai`) is deployed on the BigFile main network, you could run a command that looks like this to return events:
```
