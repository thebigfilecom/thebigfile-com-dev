...
As discussed in [tokens and cycles](/docs/current/concepts/tokens-cycles), BF tokens can be converted into **cycles** to power <GlossaryTooltip>cube</GlossaryTooltip> operations. Cycles reflect the operational cost of communication, computation, and storage that dapps consume.

Unlike BF tokens, cycles are only associated with cubes and not with user or developer <GlossaryTooltip>principals</GlossaryTooltip>. Because only cubes require cycles to perform operations and pay for the resources they use, users and developers can manage the distribution and ownership of cycles through a special type of canister called a **cycles wallet**.
...
If you are deploying on the BigFile mainnet, you typically create your cycles wallet by converting BF tokens to cycles, transferring the cycles to a new cube principal, and updating the cube with the default cycles wallet WebAssembly module (Wasm).
...
```
dfx ledger --network ic create-canister <principal-identifier> --amount <bf-tokens>
```
This command converts the number of BF tokens you specify for the `--amount` argument into cycles, and associates the cycles with a new cube identifier controlled by the principal you specify.

For example, the following command converts .25 BF tokens into cycles and specifies the principal identifier for the default identity as the controller of the new cube:
```
