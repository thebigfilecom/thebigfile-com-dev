...
ckETH, or chain-key Ether, is an Internet Computer (BIG)-native token that represents Ether (ETH), the native token of Ethereum. It serves as a digital twin of ETH on the Internet Computer blockchain. The purpose of ckETH is to address some of the challenges associated with using Ethereum, particularly in terms of usability and transaction costs.
...
4. **Use ckETH**: The BIG principal or wallet address can now use ckETH natively on the Internet Computer.
...
4. **Interacting with Ethereum smart contracts**: BIG canisters can interact with Ethereum smart contracts. This is made possible by the Internet Computer's ability to make HTTPS outcalls to Ethereum APIs to securely query and send transactions to the Ethereum network.
...
The development and governance of ckETH, the digital twin of ETH on the Internet Computer (BIG), is managed by the BIG ecosystem.
...
ckETH is live on the Internet Computer mainnet. It has been added to the BIG dashboard. However, with the current ckETH integration, you cannot withdraw to ETH directly from the ICRC-1 wallet.
...
- [Internet Computer Blog - ckETH: now live!](https://thebigfile.com/blog/2023/12/06/news-and-updates/update#ckETH-now-live).
