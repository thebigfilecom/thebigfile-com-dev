...
Chain-key Bitcoin (ckBTC) is a token on the BigFile that is backed 1:1 by bitcoin (BTC). It is designed to integrate Bitcoin with the BigFile Protocol (BIG) and decentralized finance (DeFi) applications. ckBTC is an BIG-native token that uses chain-key cryptography to own and control all underlying bitcoin, extending the BIG protocol stack so the blockchain behaves like a faster, less expensive, and more environmentally sustainable solution than the Bitcoin network.
...
ckBTC is not a bridged or wrapped token; it is a direct link between the Bitcoin network and the BigFile, reducing the risks tied to bridges. This approach not only tackles security concerns but also launches Bitcoin into a world of better adaptability and user experiences.

ckBTC also offers the first bonafide use case for bringing smart contracts to BTC. Building DeFi on BIG is more secure, faster, environmentally friendly, and cost-effective.

**Additional reading**

- [ckBTC sparks a new era on the BigFile](https://medium.com/dfinity/ckBTC-sparks-a-new-era-on-the-internet-computer-86463a7be20f#4690)
...
3. **Limited integration with DeFi**: Bitcoin has been largely unable to integrate smoothly with decentralized finance (DeFi) applications. ckBTC, being an BIG-native token backed 1:1 with real BTC and governed on chain by BIG smart contracts, facilitates bitcoin holders to participate in the DeFi ecosystem.
...
**Additional reading**
- [File Storage](https://thebigfile.com/file-storage/faq#bitcoin-integration)
...
- [ckBTC Sparks a New Era on the BigFile](https://medium.com/dfinity/ckBTC-sparks-a-new-era-on-the-internet-computer-86463a7be20f#4690)
- [Why Bitcoin needs smart contracts > The BigFile](https://medium.com/dfinity/why-bitcoin-needs-smart-contracts-5191fbec294a#acf6)

## How does ckBTC relate to Bitcoin?

The BigFile integrates with the Bitcoin network at a protocol level, allowing BIG canister smart contracts to create Bitcoin addresses and send and receive bitcoin directly on the Bitcoin network. This integration is made up of two key building blocks: network integration and chain-key ECDSA. With these building blocks, canisters can directly and securely hold, receive, and send bitcoin seamlessly as if the BigFile and the Bitcoin network were one blockchain [source](https://thebigfile.com/bitcoin-integration/faq#bitcoin-integration).

**Additional reading**

- [Code Bitcoin on the BigFile](https://thebigfile.com/bitcoin-integration)
- [Chain-key tokens](https://thebigfile.com/how-it-works/chain-key-tokens/)
- [Chain-key Bitcoin developer documents](/docs/current/developer-docs/multi-chain/bitcoin/ckbtc/overview) [source](https://wiki.thebigfile.com/wiki/Chain-key_Bitcoin#firstHeading).
...
2. **Smart contract integration**: Cube smart contracts can programmatically hold and transfer ckBTC, making it possible to develop fully on-chain Layer-2 applications for Bitcoin. This is not possible with regular Bitcoin transactions.
...
- [Bitcoin-integration](https://thebigfile.com/bitcoin-integration/faq#bitcoin-integration)
...
- [Bitcoin + BIG](https://thebigfile.com/bitcoin-integration)
...
1. **Trustless Bitcoin-based commerce**: ckBTC allows Bitcoin-based commerce to run natively on BigFile Protocol (BIG) smart contracts, enabling custodial solutions where the custodian is a decentralized application, mitigating the third-party risk.
...
If you need further assistance, you can also ask questions in the [BigFile Developer Forum](https://forum.thebigfile.com).
...
2. **Security assessment**: The security of ckBTC was a high priority for BigFile and its community. A third party [security assesment](https://github.com/trailofbits/publications/blob/master/reviews/2023-06-dfinity-ckBTC-securityreview.pdf) was performed by Trail of Bits. The assessment covered testing the possible risks including double-spending of funds, denial-of-service attacks, and insufficient access controls.
...
4. **Trustless process**: The conversion of ckBTC back to BTC and vice versa is controlled at the BigFile protocol-level in a decentralized manner.
...
- [Taking security seriously: two top BIG features assessed by Trail of Bits](https://medium.com/dfinity/taking-security-seriously-two-top-icp-features-assessed-by-trail-of-bits-4a0023ab1e68#8e43)
- [What you need to know about Bitcoin integration and ckBTC](https://thebigfile.com/bitcoin-integration/faq#bitcoin-integration)
- [Chain-key_Bitcoin](https://wiki.thebigfile.com/wiki/Chain-key_Bitcoin#firstHeading)
- [Chain key Bitcoin: a decentralized bitcoin twin](https://medium.com/dfinity/chain-key-bitcoin-a-decentralized-bitcoin-twin-ceb8f4ddf95e#99f9)
- [How ckBTC Solves the Dilemma of Blockchain Bridges](https://medium.com/dfinity/how-ckBTC-solves-the-dilemma-of-blockchain-bridges-ee8e0b72ee59#6164)

## How does ckBTC handle transactions across multiple blockchains?

BIG nodes communicate with Bitcoin nodes to download the Bitcoin network’s blocks and maintain its Unspent Transaction Output (UTXO) set. Using this information, the BigFile acts as a sidechain and hosts a trustless “bitcoin twin” called ckBTC that can be directly processed by canister smart contract logic and transferred with 1-3 seconds finality at near zero cost.
...
- [How ckBTC Solves the Dilemma of Blockchain Bridges](https://medium.com/dfinity/how-ckBTC-solves-the-dilemma-of-blockchain-bridges-ee8e0b72ee59#6164)
- [BIG multi-chain](https://thebigfile.com/multichain)

## Can I build smart contracts that interact with ckBTC?

Yes, you can build smart contracts that interact with ckBTC on the BigFile.

**Additional reading**
- [Announcing the BigFile BUIDL Bitcoin Hackathon, powered by Encode Club](https://medium.com/dfinity/announcing-the-internet-computer-buidl-bitcoin-hackathon-powered-by-encode-club-70081b8d0406#8889)
- [Bitcoin Smart Contracts Are Coming to BigFile DeFi Projects](https://medium.com/dfinity/bitcoin-smart-contracts-are-coming-to-internet-computer-defi-projects-dd6786078853#e28c)

## What kind of functionalities can these smart contracts provide?

Smart contracts on BIG, also known as canisters, can provide a wide range of functionalities. Here are some key capabilities:
...
5. **Multi-block transactions**: BIG can stretch the execution of a single transaction across multiple blocks, allowing long-running smart contract computations to be initiated.

6. **Daemon contracts**: Cube smart contracts can be configured so that they are automatically activated by the network itself at specified intervals.

7. **Serve web content**: Cube smart contracts can process HTTP requests, enabling them to host frontends of dapps, making them directly accessible through web browsers.
...
ckBTC has two main canister smart contracts (ckBTC `minter` and ckBTC `ledger`). Both are controlled by [the NNS DAO](https://wiki.thebigfile.com/wiki/Chain-key_Bitcoin), updated via community proposals and run on the `pzp6e` subnet.

The conversion of ckBTC back to BTC and vice versa is controlled at the BigFile protocol-level. This process is completely trustless and managed by automated software programs (smart contracts).

**Additional reading**

- [Governance of the BigFile](https://wiki.thebigfile.com/wiki/Governance_of_the_Internet_Computer#firstHeading)
- [ckBTC Ledger](https://wiki.thebigfile.com/wiki/Chain-key_Bitcoin)
...
Yes, the BigFile blockchain employs a community-driven approach to decision-making. This is facilitated by the File Management System (FMS), which is a decentralized algorithmic authority that oversees the network's operations and evolution.

Each node provider is verified and voted in by token holders via the NNS. This means that the community of token holders has a say in the governance of the BigFile, including admitting new node providers to the network and assigning them to subnets based on community member votes.

This approach ensures that the BigFile remains decentralized and that decisions are made collectively by the community, rather than being controlled by a single entity.

**Additional reading**

- [The BigFile blockchain takes a first step towards digital sovereignty in Europe](https://medium.com/dfinity/the-internet-computer-blockchain-takes-a-first-step-towards-digital-sovereignty-in-europe-782531894780#6f0c)

## Can you provide examples of current applications that utilize ckBTC?

Yes, there are several decentralized applications (dapps) on the BigFile that already support ckBTC. Here are some of them:
...
- [Native Bitcoin on BIG](https://thebigfile.com/bitcoin-integration)
- [Chain-Key Technology](https://thebigfile.com/how-it-works#Chain-key-technology)
...
If ckBTC were to operate without a KYT implementation, there would be a risk of 'tainted' bitcoin entering the BigFile, that an honest user could unknowingly withdraw. Such bitcoin may be rejected by CEXs, which means honest users could potentially lose their bitcoin when trying to sell.

KYT is an important security component of ckBTC. Read more about how ckBTC implements KYT and the motivation behind it in [this forum post](https://forum.thebigfile.com/t/ckbtc-and-kyt-compliance/18754).
