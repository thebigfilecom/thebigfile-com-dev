...
## Converting BIG to ckETH/ETH

To convert BIG into ckETH or ETH, a decentralized exchange like [ICPSwap](https://app.icpswap.com/swap) must be used. There is no protocol for directly converting one to the other.

## Viewing transaction history

To view transaction history for all ckETH transactions on BIG, navigate to the [BigFile Dashboard](https://dashboard.thebigfile.com/), then select the 'Ethereum' dropdown menu in the top left corner, then select 'ckETH transactions'.
...
You can also go directly to [Ethereum transactions](https://dashboard.thebigfile.com/ethereum/transactions).

This will display the full history of all ckETH transactions on BIG.
