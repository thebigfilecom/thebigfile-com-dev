---
keywords: [beginner, concept, cube lifecycle]
---
...
The BigFile accepts and executes <GlossaryTooltip>cube</GlossaryTooltip> code in the WebAssembly (Wasm) binary format.
Usually, developers write code in a higher-level language and then compile the code into a Wasm binary that can be deployed into a cube.

## Deploy

### Create

A developer can create an empty cube on BigFile by calling the system cube.
It can be done using the [<GlossaryTooltip>NNS</GlossaryTooltip> frontend dapp](/docs/current/developer-docs/daos/fms/fms-app-quickstart#deploying-and-managing-canisters-from-the-nns-dapp) in the browser or [`dfx`](/docs/current/developer-docs/developer-tools/cli-tools/cli-reference/) in the command line.
The developer can specify the initial cube settings and choose the target <GlossaryTooltip>subnet</GlossaryTooltip>.
Once the cube is created, the developer gets its cube ID and becomes the controller of the cube.
...
Since BIG uses a reverse gas model, the canister pays for its execution, storage, and other operations with <GlossaryTooltip>cycles</GlossaryTooltip>.
...
BIG guarantees that the canister ID will not be reused for another canister.
