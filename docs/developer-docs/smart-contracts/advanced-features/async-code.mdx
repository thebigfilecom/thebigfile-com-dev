...
On BIG, `async/await` is useful in the context of inter-canister calls. Inter-canister calls cannot be handled synchronously (as the two canisters
...
the callee is handled “synchronously”. However, given BIG's message passing model, one has to be careful in case there are [concurrent calls that
...
In order to implement inter-canister calls on BIG, there would need to be a future which, once created, takes care of calling
