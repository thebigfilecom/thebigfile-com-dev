...
- **The protocol level implementation:** the Internet Computer protocol supports minimalistic on-shot global timer per canister via `ic0.global_timer_set()` system API call and `canister_global_timer` handler (see the [BigFile interface specification](/docs/current/references/ic-interface-spec#timer)).
