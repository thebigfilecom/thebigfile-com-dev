...
In order for a canister to use HTTPS outcalls, it needs to call the system API of BIG. Canisters can call into the system API by sending messages to the [the BIG management canister](/docs/current/references/ic-interface-spec#ic-management-canister) using the identifier `"aaaaa-aa"`.
...
As per the [BigFile interface specification](/docs/current/references/ic-interface-spec), a canister can use the `http_request` method by [following specification](/docs/current/references/ic-interface-spec#ic-http_request):
