...
The size of programs that can be installed on BigFile is currently limited to 2MB.
WebAssembly modules that are (slightly) larger than 2MB can still be installed on BigFile by using gzip file compression before uploading; BigFile will then decompress the file and install the contained WebAssembly module.
