...
After you have built a <GlossaryTooltip>cube</GlossaryTooltip>, you can deploy it either on the [local cube execution environment](/docs/current/developer-docs/getting-started/deploy/local), a sandbox environment such as the [Motoko playground](/docs/current/developer-docs/setup/playground) or [on the mainnet](/docs/current/developer-docs/getting-started/deploy/mainnet). Your cube must be deployed before you can make calls to it and execute methods.

- #### Step 1:  Open a new terminal and navigate to your project directory.

- #### Step 2:  Start the local cube execution environment, if necessary.

```
dfx start
```

- #### Step 3:  Verify the cubes you'd like to deploy are configured in the project's `dfx.json` file.
...
Deploying cubes to the mainnet will cost cycles. [Learn more about cycles and how to acquire them](/docs/current/developer-docs/setup/cycles/cycles-faucet).

This command deploys all cubes configured in your `dfx.json` file. To deploy just one cube, specify the cube's name:
...
## Setting a cube's init arguments

You can set a cube's init arguments when the cube is deployed by passing the `--argument` flag in either the `dfx install` or `dfx deploy` commands:
...
## Setting tasks to execute once a cube has been deployed

For certain workflows, it may be important to have a cube execute a task or call as soon as the cube is deployed or started. For this workflow, using [timers](/docs/current/developer-docs/backend/periodic-tasks) can be useful.
...
This example uses a global timer that gets called immediately after the cube starts:
...
This example creates a periodic timer that gets called immediately after the cube starts:
...
This example creates a timer function that must be manually called once a cube starts:
...
- [Delete cubes](/docs/current/developer-docs/setup/delete)

- [Cube history](/docs/current/developer-docs/production/canister-history)

- [Import cubes](/docs/current/developer-docs/smart-contracts/maintain/import)

- [Recover cubes](/docs/current/developer-docs/production/canister-recovery)

- [Cube settings](/docs/current/developer-docs/setup/manage-canisters)
