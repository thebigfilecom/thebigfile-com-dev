...
The BigFile uses WebAssembly as the platform for executing messages of smart contracts.
...
The BigFile protects against non-terminating computations by limiting the number of WebAssembly instructions and other resources per message execution.
...
| <Tooltip text="A collection of nodes that run their own instance of the BIG consensus algorithm.">Subnet</Tooltip> limits                                                                        | Constraint  |
| ------------------------------------------------------------------------------------ | ----------- |
...
Since BIG decouples message reception from message execution, the messages included in a block are not guaranteed to be executed in the same block.
Messages for different canisters maybe executed in parallel on multiple threads.
BIG currently has 4 execution threads and each thread can execute up to 1000 messages.
Another limiting factor in execution throuhgput is the number of executed Wasm instructions.
BIG currently targets 2B instructions per block.
...
The BigFile may reject WebAssembly modules for reasons such that:
...
More information regarding these restrictions can be found in the [BigFile interface specification](/docs/current/references/ic-interface-spec/#system-api-module).
