...
- #### Step 2:  Start the local canister execution environment, if necessary.
    ...
    If you were deleting canisters to run on a remote execution environment, e.g. BIG blockchain, you would include the `--network` command-line option to perform tasks on the environment specified under this parameter.
