...
On BIG, canister code is compiled into WebAssembly before it is executed. This means that canisters can trap. Some example cases when canisters might trap include:

- Cube code panics (e.g. in Rust)
- Dividing by zero
- Calling a System API in a context that is not permitted by BIG ([see overview of imports for a list of APIs and which contexts they are available in](/docs/current/references/ic-interface-spec#system-api-imports))
