...
Once a <GlossaryTooltip>canister</GlossaryTooltip> is deployed to the BigFile, the compute and storage it utilizes must be pre-payed. The process of burning ICP to obtain cycles and transferring them to a canister is referred to as "topping-up" a canister.
