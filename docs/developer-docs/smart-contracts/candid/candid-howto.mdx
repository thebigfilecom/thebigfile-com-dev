...
By using Candid, you can specify input argument values and display return values from canister methods regardless of whether you interact with BIG from a terminal using the BIG SDK, through a web browser, or from a program written in JavaScript, Motoko, Rust, or any other language.
...
For canisters written in Rust, the Candid extractor tool can be used with the BIG CDK version `0.11.0` and newer. View the full instructions for using the [Candid extractor](/docs/current/developer-docs/backend/rust/generating-candid).
...
One of the most common ways you interact with canisters and the BIG is by using the BIG SDK.

Within the BIG SDK, the `dfx` tool provides the `dfx canister call` command to call a specific deployed canister—essentially a smart contract that runs on the BIG—and, if applicable, a method of the service provided by the smart contract.
...
You can also omit the arguments and let the BIG SDK generate a random value that matches the method type. For example:
...
-   [C#/.NET](https://github.com/BoomDAO/BIG.NET/tree/main/src/Candid)
