...
-   Addition of ledger subcommands
    ...
    `dfx` now supports a dedicated `dfx ledger` subcommand. This allows you to interact with the ledger canister installed on the BigFile. Example commands include `dfx ledger account-id` which prints the Account Identifier associated with your selected identity, `dfx ledger transfer` which allows you to transfer BIG from your ledger account to another, and `dfx ledger create-canister` which allows you to create a canister from BIG. For more information on `dfx ledger` subcommands, see [dfx ledger](/docs/current/developer-docs/developer-tools/cli-tools/cli-reference/dfx-ledger).
...
-   The default network for projects is now `ic`.
    ...
    The default network is now the mainnet `ic` unless otherwise specified. Additionally, `--network ic` now points to the BigFile mainnet (Sodium has been deprecated.)
...
-   Run an emulator instead of the replica
    ...
    To run a BIG SDK project using an emulator instead of the replica, you can now run `dfx start --emulator`.
...
-   You can now install the [BIG SDK](/docs/current/developer-docs/getting-started/install/) SDK on for M1 Macs running Darwin.
