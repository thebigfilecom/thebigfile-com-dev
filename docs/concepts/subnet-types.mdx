...
To top up your principal's balance, you can send cycles to the principal using the [FMS dapp](https://fms.thebigfile.com/wallet/), or [learn how to convert BIG tokens into cycles](/docs/current/tutorials/developer-journey/level-1/1.4-using-cycles#converting-icp-tokens-to-cycles).
...
Adjust the `--amount 0.5` to the number of BIG tokens that you want to send the new canister. More information can be found in the [documentation](/docs/current/developer-docs/developer-tools/cli-tools/cli-reference/dfx-ledger/#options). 
