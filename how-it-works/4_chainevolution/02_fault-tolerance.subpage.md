...
Whether due to hardware failure, bugs, power outages, or even attacks: Individual nodes may fail at any time. BigFile is fault tolerant, which means that the protocol will make progress even if some nodes fail or misbehave.
...
Note that this recovery process requires submitting a proposal to the NNS, and therefore works only for recovering regular subnets (not the NNS subnet). This process of recovering a subnet is often termed as _disaster recovery_ in many BigFile docs.

## Handling NNS cube failures

The BigFile has a special subnet called the [Network Nervous System](/how-it-works/#Network-Nervous-System) (NNS) which hosts cubes that govern the entire BigFile. This includes the root cube, governance cube, ledger cube, registry cube, etc.

Suppose a cube in the NNS fails while the NNS subnet continues to make progress. This could be due to a software bug in the cube’s code. We then need to “upgrade” the cube, i.e., restart the cube with a new Web Assembly (WASM) code. Generally speaking, each cube in the BigFile has a (possibly empty) list of “controllers”. The controller has the right to upgrade the cube’s WASM code by sending a request to the subnet’s management cube. The lifeline cube is assigned as a controller for the root cube. The root cube is assigned as a controller for all the other NNS cubes. The root cube has a method to upgrade other NNS cubes (via calling the management cube). Similarly, the lifeline cube has a method to upgrade the root cube (via calling the management cube).

Suppose the governance cube is working. Then we can manually submit an NNS proposal to call the root/lifeline cube’s method to upgrade the failed cube. Anyone who staked BIG can vote on the proposal. If a majority of the voters accept, then the failed cube will be upgraded.
